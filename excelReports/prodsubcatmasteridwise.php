...
->setCellValue('B1', 'Product SubCat Id')

->setCellValue('C1', 'Product SubCat Name')

->setCellValue('D1', 'Product Category Id')
...
$sql=mysqli_query($link1,"Select * from product_sub_category where ".$status." and ".$prodcat." order by prod_sub_cat")or die("er1".mysqli_error($link1));
...
if($row_loc['status'] == '1'){ $status =  "Active";} else { $status =  "Deactive";}
	...
	//$updateby = getAnyDetails($row_loc['updateby'],"name","username","admin_users",$link1);
		...
		$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('B'.$i, $row_loc['psubcatid'])
				
				->setCellValue('C'.$i, $row_loc['prod_sub_cat'])

				->setCellValue('D'.$i, $row_loc['productid'])
					
				->setCellValue('E'.$i, $row_loc['product_category'])
...
$objPHPExcel->getActiveSheet()->getStyle('A1:F'.($i-1))->applyFromArray($styleArray);
...
header('Content-Disposition: attachment;filename="productsubcategorycatidlist.xlsx"');
