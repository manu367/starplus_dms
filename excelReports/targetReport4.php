...
<table width="100%" border="1" cellpadding="2" cellspacing="1" bordercolor="#000000">
    <tr align="left" style="background-color:#396; color:#FFFFFF;font-size:13px;font-family:Verdana, Arial, Helvetica, sans-serif;font-weight:normal;vertical-align:central">
        ...
        <!--<th>Remark</th>-->
        <th>Target Value</th>
		<th>Target (Primary/Old)</th>
		<th>Target (Secondary/New)</th>
        <!--<th>Achivement - Self</th>-->
		<th>Achivement - Self (Primary/Old)</th>
		<th>Achivement - Self (Secondary/New)</th>
        <!--<th>Achivement - Team</th>-->
		<th>Achivement - Team (Primary/Old)</th>
		<th>Achivement - Team (Secondary/New)</th>
		<th>Achivement% (Primary/Old)</th>
		<th>Achivement% (Secondary/New)</th>
    </tr>
<?php
			...
			if(mysqli_num_rows($invcnt_res) > 0){
				...
				while($row = mysqli_fetch_assoc($invcnt_res)){
					...
					if($dmode){
						...
						// DV(Old)/SO(Primary)
						$desc_self_p = ($row['task_name'] == 'Dealer Visit')?(int)$ac_arr[0]:(($row['task_name'] == 'Sale Order')?(int)$ac_arr[0]:("n/a"));
						// DV(New)/SO(Secondary)
						$desc_self_s = ($row['task_name'] == 'Dealer Visit')?(int)$ac_arr[1]:(($row['task_name'] == 'Sale Order')?(int)$ac_arr[1]:("n/a"));
					...
					if($dmode){
						...
						//DV(Old)/SO(Primary)
						$desc_team_a = ($row['task_name'] == 'Dealer Visit')?$t_a:(($row['task_name'] == 'Sale Order')?$t_a:("n/a"));
						//DV(New)/SO(Secondary)
						$desc_team_b = ($row['task_name'] == 'Dealer Visit')?$t_b:(($row['task_name'] == 'Sale Order')?$t_b:("n/a"));
					...
					$p_target = str_replace("Old-","", $remark_arr[0]);
					$s_target = str_replace("New-","", $remark_arr[1]);				
					...
					//echo '<tr><td>'.$count.'</td><td>'.$userinfo.'</td><td>'.$row['emp_id'].'</td><td>'.$desig.'</td><td>'.$depart.'</td><td>'.$subdepart.'</td><td>'.$row['year'].'</td><td>'.$row['month'].'</td><td>'.$row['prod_code'].'</td><td>'.$row['task_name'].'</td><td>'.$row['remark'].'</td><td>'.$row['target_val'].'</td><td>'.$p_target.'</td><td>'.$s_target.'</td><td>'.(($achivement)?(int)$achivement:0).'</td><td>'.$desc_self_p.'</td><td>'.$desc_self_s.'</td><td>'.$total_ac.'</td><td>'.$desc_team_a.'</td><td>'.$desc_team_b.'</td><td>'.$p_per.'%</td><td>'.$s_per.'%</td></tr>';
					//echo '<tr><td>'.$_SESSION['tempcount'].'</td><td>'.$userinfo.'</td><td>'.$row['emp_id'].'</td><td>'.$desig.'</td><td>'.$depart.'</td><td>'.$subdepart.'</td><td>'.$row['year'].'</td><td>'.$row['month'].'</td><td>'.$row['prod_code'].'</td><td>'.$row['task_name'].'</td><td>'.$row['target_val'].'</td><td>'.$p_target.'</td><td>'.$s_target.'</td><td>'.$desc_self_p.'</td><td>'.$desc_self_s.'</td><td>'.$desc_team_a.'</td><td>'.$desc_team_b.'</td><td>'.$p_per.'%</td><td>'.$s_per.'%</td></tr>';
					
					echo '<tr><td>'.$_SESSION['tempcount'].'</td><td>'.$userinfo.'</td><td>'.$row['emp_id'].'</td><td>'.$desig.'</td><td>'.$depart.'</td><td>'.$subdepart.'</td><td>'.$row['year'].'</td><td>'.$row['month'].'</td><td>'.$row['prod_code'].'</td><td>'.$row['task_name'].'</td><td>'.$row['target_val'].'</td><td>'.(($dmode)?$p_target:$row['target_val']).'</td><td>'.(($dmode)?$s_target:'').'</td><td>'.(($dmode)?$desc_self_p:$total_ac).'</td><td>'.(($dmode)?$desc_self_s:'').'</td><td>'.(($dmode)?$desc_team_a:'').'</td><td>'.(($dmode)?$desc_team_b:'').'</td><td>'.(($dmode)?$p_per:$per).'%</td><td>'.(($dmode)?$s_per.'%':'').'</td></tr>';
					
					
					
					
					$_SESSION['tempcount'] = $_SESSION['tempcount']+1;
				}
			}
			else{
				//echo "<br>NO task!<br>";
