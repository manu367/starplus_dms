...
//////End filters value/////
if($potype){ $sale_type = " AND a.sale_type='".$potype."'";}else{ $sale_type = "";}
if($psc){ $psc_cat = " AND a.psc_id='".$psc."'";}else{ $psc_cat = "";}
//echo "SELECT b.* FROM purchase_order_data a INNER JOIN purchase_order_master b ON a.po_no=b.po_no WHERE ".$from_party." AND ".$to_party." AND ".$sql_date." AND ".$product_code." AND ".$status." ".$sale_type." ".$psc_cat." ".$statt." ".$rsm;
$sql=mysqli_query($link1,"SELECT b.* FROM purchase_order_data a INNER JOIN purchase_order_master b ON a.po_no=b.po_no WHERE ".$from_party." AND ".$to_party." AND ".$sql_date." AND ".$product_code." AND ".$status." ".$sale_type." ".$psc_cat." ".$statt." ".$rsm)or die("er1".mysqli_error($link1));
?>
