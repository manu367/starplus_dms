...
## selected location
if($location!=""){
	$loc="from_location='".$location."'";
}else{
	$locstr=getAccessLocation($_SESSION['userid'],$link1);
	$loc="from_location in (".$locstr.")";
}
//////End filters value/////
$sql = mysqli_query($link1,"SELECT * FROM billing_master where ".$loc." and (status='Received' or status='Dispatched') and is_adjust!='Y'")or die("er1".mysqli_error($link1));
?>
...
while($row_loc = mysqli_fetch_array($sql)){
	$new_loc = $row_loc['from_location'];
	$new_loc2 = $row_loc['to_location'];
	if($old_loc != $new_loc){
		$locdet=explode("~",getLocationDetails($row_loc['from_location'],"name,city,state,id_type",$link1));
	}
	if($old_loc2 != $new_loc2){
		$locdet2=explode("~",getLocationDetails($row_loc['to_location'],"name,city,state,id_type",$link1));
	}
?>
<tr>
<td align="left"><?=$i?></td>
...
<?php
$old_loc = $row_loc['from_location'];
$old_loc2 = $row_loc['to_location'];
$i+=1;		
}
?>
</table>