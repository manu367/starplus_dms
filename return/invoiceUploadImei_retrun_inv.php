				...
				foreach( $xlsx->rows() as $k => $r) {
					...
					for( $i = 0; $i < count($k); $i++)
					{
						...
						if($r[0]=='' && $r[1]=='' && $r[2]==''){
							...
							//echo  "select id,owner_code from billing_imei_data where (imei1='" . $imei1 . "' or imei2='" .$imei1 . "' or imei1='" . $imei2 . "' or imei2='" .$imei2 . "')  and prod_code ='" . $partcode. "'  order by id desc";
							//exit;										
							$imeidata = mysqli_fetch_assoc(mysqli_query($link1, "select id,owner_code from billing_imei_data where imei1='" . $r[1] . "' AND prod_code ='" . $partcode . "'   order by id desc"));
							if ($imeidata['id'] != "" && $imeidata['owner_code']==$froml) {
							
							} else {
								$msg_imei = "Serial no. does not exist in database!".$imei1."-".$froml."-".$imeidata['owner_code'];
								header("Location:purchase_return.php?challan_no=" . $_POST['challan_no'] . "&msg=$msg_imei" . $pagenav);
								exit;
							}				
							if ($partcode != "" && $imei1 != "" && $imei2 != "") {		
								$owner_code1 =mysqli_query($link1, "select owner_code,import_date from billing_imei_data where prod_code='" .$partcode . "' and  imei1='" . $imei1 . "' order by id desc");	
								...
								if(rtrim($row1['owner_code']," ")==$froml){			                 	               
									...
									$sql = "INSERT INTO billing_imei_data set from_location='" .$froml . "',to_location='" . $tol . "',owner_code='" .$owner_no . "',prod_code='" .$partcode. "',doc_no='" . $_POST['challan_no'] . "',imei1='". $imei1 . "',stock_type='" . $imei2 . "',flag='" . $flag . "', transaction_date='".$vendor_master["sale_date"]."', import_date='".$row1["import_date"]."' ";
									$result = mysqli_query($link1, $sql);                       
										...
										if (!$result) {
											...
											/*$result3  =  mysqli_query($link1, "update billing_model_data set file_name='" . $f_name . "',imei_attach='Y' where challan_no='" . $_POST['challan_no'] . "' and prod_code = '".$partcode. "'  ");
											//// check if query is not executed
											if (!$result3) {
												$flag = false;
                        ...
                        <form  name="frm1"  id="frm1" class="form-horizontal" action="" method="post"  enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <div class="col-md-12" align="center">
                                    ...
                                    <input title="Back" class="btn <?=$btncolor?>" value="Back" onClick="window.location.href = 'purchase_return.php?op=edit&id=<?= ($_REQUEST['id'])?><?= $pagenav ?>'" type="button">
                                </div>
                            </div> 
                        </form>
                    </div>
