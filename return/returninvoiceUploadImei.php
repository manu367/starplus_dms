...
if ($_POST['Submit'] == "Upload") {
    ...
    if ($_FILES["attchfile"]["name"]) {
		...
		///// pick perticular part details
			$vendor_data = mysqli_fetch_assoc(mysqli_query($link1,"select qty  from billing_model_data where challan_no='".$_POST['challan_no']."' and 
	...
	if($vendor_data['qty'] == ($total_count -1)){
						 ...
						 if ($k == 0) continue; // skip first row 
						  for( $i = 0; $i < count($k); $i++)
						  {
							  ...
							  if($r[0]=='' && $r[1]=='' && $r[2]==''){
								 ...
								 ////Make Variable for each element of excel//////		
									   ...
									   //// $po_no = $vendor_master['po_no'];
										$flag = "Y";
										  ...
										  $part_code = mysqli_fetch_assoc(mysqli_query($link1, "select prod_code  from billing_model_data where prod_code='" .$_POST['prodcode']  . "' and challan_no='" . $_POST['challan_no'] . "' group by prod_code "));
										if ($part_code['prod_code'] != "") {
											$partcode = $r[0];
										} else {
											$msg_partcode = "Product code not exit!";
											header("Location:purchase_return.php?challan_no=" . $_POST['challan_no'] . "&msg=$msg_partcode" . $pagenav);
											exit;
										}
					
									   // check imei1 and imei2
									  /// echo "select id,owner_code from billing_imei_data where (imei1='" . $r[1] . "' or imei2='" . $r[1] . "' or imei1='" . $r[2] . "' or imei2='" . $r[2] . "')  and prod_code ='" . $partcode . "'  order by id desc";
										
										$imeidata = mysqli_fetch_assoc(mysqli_query($link1, "select id,owner_code from billing_imei_data where (imei1='" . $imei1 . "' or imei2='" .$imei1 . "' or imei1='" . $imei2 . "' or imei2='" .$imei2 . "')  and prod_code ='" . $_POST['prodcode']. "'  order by id desc"));
									
										if ($imeidata['id'] != "" && $imeidata['owner_code']==$froml) {
											
										} else {
											$msg_imei = "IMEI Does not exist in database!";
											header("Location:purchase_return.php?challan_no=" . $_POST['challan_no'] . "&msg=$msg_imei" . $pagenav);
											exit;
										}
									
									$imeidata = mysqli_fetch_assoc(mysqli_query($link1,"select id from billing_imei_data where (imei1='" .$imei1. "' or imei2='" .$imei2. "' or imei1='" .$imei2. "' or imei2='" .$imei1. "'  and prod_code ='" . $_POST['prodcode']. "')  order by id desc"));
									
									
										if ($imeidata['id']== "") {
										   $msg_imei = "IMEI is Already Uploaded";
										   header("Location:purchase_return.php?op=edit&id='". $_POST['challan_no']."' &msg=$msg_imei" . $pagenav);
										   exit; 
										} 
										...
										if ($_POST['prodcode'] != "" && $imei1 != "" && $imei2 != "") {		
											$owner_code1 =mysqli_query($link1, "select owner_code from billing_imei_data where prod_code='" .$_POST['prodcode'] . "' and  (imei1='" . $imei1 . "' or imei2='" .$imei1 . "' or imei1='" . $imei2 . "' or imei2='" .$imei2 . "') order by id desc");	
												$row1 = mysqli_fetch_array($owner_code1);
												///echo $row1['owner_code'].".=.".$froml."</br>";								
														if(rtrim($row1['owner_code']," ")==$froml){			                 	               
												// insert imei into billing_imei_data and billing_imei
												$sql = "INSERT INTO billing_imei_data set from_location='" .$froml . "',to_location='" . $tol . "',owner_code='" .$owner_no . "',prod_code='" .$_POST['prodcode']. "',doc_no='" . $_POST['challan_no'] . "',imei1=" . $imei1 . ",imei2='" . $imei2 . "',flag='" . $flag . "' , stock_type = '".$stocktype."' ";
								...
								$result3  =  mysqli_query($link1, "update billing_model_data set file_name='" . $f_name . "',imei_attach='Y' where challan_no='" . $_POST['challan_no'] . "' and prod_code = '".$_POST['prodcode']. "'  ");
        ...
        if ($msg == '1') {
            ...
            if ($flag1) {
                ...
                $f_name = $now . $_FILES["attchfile"]["name"];
                // insert into upload file data
          $status= mysqli_fetch_array(mysqli_query($link1, "select count(id) as vid from billing_model_data where imei_attach = 'Y'  and challan_no='".$_POST['challan_no']."' "));
		  $po_no = mysqli_fetch_array(mysqli_query($link1, "select count(id) as vid from billing_model_data where  challan_no = '".$_POST['challan_no']."' "));
		  if($status['vid']  == $po_no['vid'])
		  {
		    $result4  =  mysqli_query($link1, "update billing_master set file_name='" . $f_name . "',imei_attach='Y'  where challan_no='" . $_POST['challan_no'] . "'   ");
					  
		  }					
                mysqli_commit($link1);
                $msg = "File is uploaded Successfully!";
             header("Location:purchase_return.php?op=edit&id='". $_POST['challan_no']."' &msg=$msg" . $pagenav);
                exit;
            } else {
        ...
        <div class="container-fluid">
            <div class="row content">
                ...
                <div class="col-sm-9 tab-pane fade in active" id="home">
                    <h2 align="center"><i class="fa fa-upload"></i>Purchase Return  Upload IMEI <br>
                        ...
                        <form  name="frm1"  id="frm1" class="form-horizontal" action="" method="post"  enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <div class="col-md-12" align="center">
                                    <input title="Back" class="btn btn-primary" value="Back" onClick="window.location.href = 'purchase_return.php?op=edit&id=<?= ($_GET['po_no'])?><?= $pagenav ?>'" type="button">&nbsp;&nbsp;&nbsp;
