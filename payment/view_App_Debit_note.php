...
$po_sql="SELECT * FROM debit_note where ref_no='".$docid."'";
	...
	if(isset($_POST['updButton'])){
	 	if ($_POST['updButton']=='Update'){
			...
			if($actiontaken == 'Approved') {
				...
				$result_ledger=mysqli_query($link1,"insert into party_ledger set location_code='".$location_code."',cust_id='".$party_name."',doc_no='".$ref_no."',doc_date='".$doc_date."', entry_date='".$today."',entry_time='".$currtime."',entry_by='".$_SESSION['userid']."',doc_type='DR NOTES',amount='".$amt."',cr_dr='DR'");
				//// check if query is not executed
				if (!$result_ledger) {
					 $flag = false;
				...
				$main = 	mysqli_query($link1,"update debit_note set app_status='$appstatus',status='$appstatus',app_date='$today',app_time='$now',app_remark='$remark',app_id ='$_SESSION[userid]' where ref_no='$ref_no'");
				############# check if query is not executed
				if (!$main) {
					$flag = false;
					$err_msg = "Error details14: " . mysqli_error($link1) . ".";
				}
				$msg="DR NOTE ".$ref_no." is ".$appstatus;
				$cflag ="success";
				$cmsg="success";
				...
				$res_data = mysqli_query($link1,"SELECT value,sgst_per,cgst_per,igst_per,sgst_amt,cgst_amt,igst_amt FROM debit_note_data WHERE ref_no='".$ref_no."'");
				...
				$resp = explode("~",storeLedgerTransaction($location_code,$ref_no,$today,"4","Debit Note",$arr_taxx,$arr_val,$tcs_per,$tcs_amt,$round_off,$gst_type,$arr_ldg_name,"Direct Purchase","Purchase Accounts",$link1,$flag));
				$flag = $resp[0];
				$error_msg = $resp[1];
			...
			mysqli_close($link1);
			///// move to parent page
			header("location:approve_debit_notes.php?msg=".$msg."&chkflag=".$cflag."&chkmsg=".$cmsg."".$pagenav);
			exit;
	    }
	}
}
...
<div class="container-fluid">
  <div class="row content">
   ...
   include("../includes/leftnav2.php");
    ?>
    <div class="col-sm-9">
      <h2 align="center"><i class="fa fa-address-card"></i> View Debit Note Detail</h2><br/>
   <form  name="frm1" id="frm1" class="form-horizontal" action="" method="post">   
   <div class="panel-group">
    <div class="panel panel-default table-responsive">
        <div class="panel-heading heading1 ">Debit Note Information</div>
         ...
         <table class="table table-bordered" width="100%">
            <tbody>
              ...
              <tr>
                ...
                <td><?php $datacust = explode("~",getCustomerDetails($bill_det["cust_id"],"customername,state",$link1));
		if($datacust[0] == ''){
			 $showdatanew1 = explode("~",getVendorDetails($bill_det["cust_id"],"name,state",$link1));
			$showdatanew = $showdatanew1[0];
			$custdet =  $showdatanew1[1];
		}
		else {
			$showdatanew = $datacust[0];
			$custdet =  $datacust[1];
			} echo $showdatanew; ?></td>
              ...
              <tr>
                ...
                <td><?php echo $bill_det['description'];?></td>
                <td><label class="control-label">Entry Date</label></td>
                <td><?=$bill_det['create_date']?></td>                
              </tr>
               <tr>
    ...
    <div class="panel panel-default table-responsive">
        ...
        <table class="table table-bordered" width="100%">
           ...
           <?php
            $i=1;
            $podata_sql = "SELECT * FROM debit_note_data where ref_no='".$docid."'";
            ...
            <tbody>
              ...
              <tr>
				...
				<input type="hidden" name="doc_date" id="doc_date" value="<?=$bill_det['create_date']?>">
                  ...
                  <input title="Back" type="button" class="btn <?=$btncolor?>" value="Back" onClick="window.location='approve_debit_notes.php?status=<?=$_REQUEST['status']?>&from_date=<?=$_REQUEST['from_date']?>&to_date=<?=$_REQUEST['to_date']?>&location_code=<?=$_REQUEST['location_code']?><?=$pagenav?>'">
                  </td>
                </tr>
            </tbody>
          </table>
          ...
          <table class="table table-bordered" width="100%"> 
            ...
            <tbody>
              <tr>
               ...
               <tr>
                <td colspan="4" align="center"><input title="Back" type="button" class="btn <?=$btncolor?>" value="Back" onClick="window.location='approve_debit_notes.php?status=<?=$_REQUEST['status']?>&from_date=<?=$_REQUEST['from_date']?>&to_date=<?=$_REQUEST['to_date']?>&location_code=<?=$_REQUEST['location_code']?><?=$pagenav?>'"></td>
                </tr>
