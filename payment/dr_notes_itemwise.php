...
$cust = explode("~",getVendorDetails($_REQUEST['vendor'],"state,city",$link1));
@extract($_POST);
...
{	
	...
	$error_msg="";
	//////////////
	/*$query_code="select MAX(sys_ref_temp_no) from debit_note where location_id='".$_POST['location_code']."'";
	$result_code=mysqli_query($link1,$query_code);
	$arr_result2=mysqli_fetch_array($result_code);
	$arr_result2[0];
	$code_id=$arr_result2[0];
	$pad=++$code_id;
	$mobiCode="DR/".$_POST['location_code']."/".$pad;*/
	$res_cnt = mysqli_query($link1, "SELECT dn_str, dn_counter FROM document_counter WHERE location_code='" . $_POST['location_code'] . "'");
	$row_cnt = mysqli_fetch_array($res_cnt);
	$invcnt = $row_cnt['dn_counter'] + 1;
	//$pad = str_pad($invcnt, 4, 0, STR_PAD_LEFT);
	$pad = $invcnt;
	$mobiCode = $row_cnt['dn_str'].$pad;
	if(mysqli_num_rows(mysqli_query($link1,"select sno from debit_note where ref_no='".$mobiCode."'"))==0){
		...
		$resultdc = mysqli_query($link1, "UPDATE document_counter SET dn_counter=dn_counter+1,update_by='" . $_SESSION['userid'] . "',updatedate='" . $datetime . "' WHERE location_code='" .$_POST['location_code']. "'");
		//// check if query is not executed
		if (!$resultdc) {
			$flag = false;
	...
	mysqli_close($link1);
	header("location:process_debit_notes.php?msg=".$msg."&chkflag=".$cflag."&chkmsg=".$cmsg."".$pagenav);
	exit;
}
?>
<!DOCTYPE html>
...
<div class="container-fluid">
  <div class="row content">
    ...
    ?>
    <div class="col-sm-9">
      <h2 align="center"><i class="fa fa-shopping-basket"></i> Itemwise Debit Note </h2><br/>
      <div class="form-group" id="page-wrap" style="margin-left:10px;">
          ...
          <div class="form-group">
            <div class="col-md-10"><label class="col-md-5 control-label">Vendor<span style="color:#F00">*</span></label>
              <div class="col-md-7">
                 <select name="vendor" id="vendor" required class="form-control selectpicker required" data-live-search="true" onChange="document.frm1.submit();">
                 <option value="" selected="selected">Please Select </option>
                    <?php 
					$sql_parent="select id, name,city,state from vendor_master where status ='Active'";
					$res_parent=mysqli_query($link1,$sql_parent);
					while($result_parent=mysqli_fetch_array($res_parent)){    
                          ?>
                    <option data-tokens="<?=$result_parent['name']." | ".$result_parent['id']?>" value="<?=$result_parent['id']?>" <?php if($result_parent['id']==$_REQUEST['vendor'])echo "selected";?> >
                       <?=$result_parent['name']." | ".$result_parent['city']." | ".$result_parent['state']." | ".$result_parent['id']?>
                    </option>
                    <?php
					}
                    ?>
          ...
          <div class="col-md-10">
              ...
              <div class="col-md-3">
                <select name="inv_no" id="inv_no" required class="form-control selectpicker required" data-live-search="true" onChange="document.frm1.submit();">
                 <option value="" selected="selected">Please Select </option>
                    <?php 
					$sql_parent="select challan_no  from billing_master where from_location='".$_REQUEST['vendor']."' and to_location='".$_REQUEST['location']."' and document_type='INVOICE'  and crdr_flag = '' ";
					$res_parent=mysqli_query($link1,$sql_parent);
					while($result_parent=mysqli_fetch_array($res_parent)){
	                      ?>
...
<!--                  Discount Type-->
		...
		<form id="frm2" name="frm2" class="form-horizontal" action="" method="post">
          ...
          <table width="100%" id="itemsTable1" class="table table-bordered table-hover">
			...
			$total_qty = 0.00;
			$toqty = 0;
			$sql=mysqli_query($link1,"Select * from billing_model_data  where from_location='".$_REQUEST['vendor']."' and challan_no='".$_REQUEST['inv_no']."'");
			...
			?>
              <tr id='addr<?php echo $i;?>'>
                ...
                <td class="col-md-1"><input type="text" class="form-control numbers" name="req_qty[<?php echo $i;?>]" id="req_qty[<?php echo $i;?>]"  autocomplete="off" required onBlur="rowTotal(0);"  value="<?php echo ($toqty);?>"></td>
                <td class="col-md-1"><input type="text" class="form-control" name="price[<?php echo $i;?>]" id="price[<?php echo $i;?>]" onBlur="rowTotal(<?php echo $i;?>);" autocomplete="off" value="<?php echo $row['price'];?>" required></td>
				...
				<td class="col-md-1"><input type="text" class="form-control" name="dis_per[<?php echo $i;?>]" id="dis_per[<?php echo $i;?>]"  autocomplete="off" onBlur="rowTotal(<?php echo $i;?>);" <?php if($_REQUEST['discount_type']!="PD"){?> disabled="disabled"<?php } ?>></td>
                <td class="col-md-1"><input type="text" class="form-control" name="rowdiscount[<?php echo $i;?>]" id="rowdiscount[<?php echo $i;?>]"  autocomplete="off" value="<?php echo $row['discount'];?>" readonly <?php if($_REQUEST['discount_type']!="PD"){?> disabled="disabled"<?php } ?>></td>
				<?php
          ...
          <div class="form-group">
            <div class="col-md-12" align="center">
              <input type="submit" class="btn btn-primary" name="upd" id="upd" value="Save" title="Save This DN">
                <input type="hidden" name="custid" id="custid" value="<?=$_REQUEST['vendor']?>"/>
              ...
              <input title="Back" type="button" class="btn btn-primary" value="Back" onClick="window.location.href='process_debit_notes.php?<?=$pagenav?>'">
            </div>
          </div>
         </form>
      </div>
...
<?php if($_REQUEST['location']=='' || $_REQUEST['vendor']=='' || $_REQUEST['discount_type'] == '' || $_REQUEST['inv_no'] == ''){ ?>
<script>
