...
$po_sql="SELECT * FROM payment_send where doc_no='".$docid."'   ";
$po_res=mysqli_query($link1,$po_sql);
$p_row=mysqli_fetch_assoc($po_res);
////// get approval data if exist
...
if (isset($_POST['upd']) && $_POST['upd']=='Save'){
	...
	if($_POST['action']=='Approve' ){
		///// update
    	$query1= "update payment_send set status='".$action."', pay_amount='".$rec_amount."' where doc_no='".$docid."' ";
		...
		if($adjust_type){
			...
			$type = "VP";
			$flag=partyLedger($_POST['parentcode'],$_POST['partycode'],$docid,$_REQUEST['pay_date'],$today, $currtime, $_SESSION['userid'],$type, $rec_amount,'DR',$link1, $flag);
			/// insert  into  location_account_ledger table //////////////////////////
   			$result3=mysqli_query($link1,"update current_cr_status set  cr_abl = cr_abl -'".$rec_amount."'  , total_cr_limit = total_cr_limit -'".$rec_amount."'   where  parent_code = '".$_POST['parentcode']."' and  asc_code = '".$_POST['partycode']."' ");
			//// check if query is not executed
			if (!$result3) {
			   $flag = false;
		...
		$flag=dailyActivity($_SESSION['userid'],$docid,"Payment Approval",$action,$ip,$link1,$flag);
	}
	//////////   case of Reject ///////////////////////////////////////////////////////////////////////////////
	else if($_POST['action']=='Reject' )  {
		///// Insert Master Data
    	$query4= "update payment_send set status='".$action."', pay_amount='".$rec_amount."' where doc_no='".$docid."' ";
	...
	if ($flag) {
        mysqli_commit($link1);
        $msg = "Payment ".$action." successfully  with ref. no.".$docid;
    } else {
    ...
    mysqli_close($link1);
	///// move to parent page
    header("location:paymentsend_approval_list.php?msg=".$msg."".$pagenav);
    exit;
 }
...
<div class="container-fluid">
          ...
          <table class="table table-bordered" width="100%">
            <tbody>
              <tr>
                ...
                <td width="30%"><?php echo str_replace("~",",",getVendorDetails($p_row['to_location'],"name,city,state",$link1));?>
  ...
  <!--<div class="row" align="center">
    ...
    <div class="col-sm-9">
      <h2 align="center"><i class="fa fa-rupee"></i> Payment Approval </h2><br/>
      <div class="form-group"  id="page-wrap" style="margin-left:10px;">
          <form id="frm1" name="frm1" class="form-horizontal" action="" method="post">
          <div class="form-group">
            <div class="col-md-10"><label class="col-md-3 control-label">Pay Amount<span style="color:#F00">*</span></label>
              <div class="col-md-9">
                 <input type="text" name="rec_amount" id="rec_amount" class="form-control number" value="<?php echo $p_row['amount'];?>" required/>
              </div>
            </div>
          </div>
          <div class="form-group">
        ...
        <div class="form-group">
            <div class="col-md-10"><label class="col-md-3 control-label">Remark<span style="color:#F00">*</span></label>
              <div class="col-md-9">
                 <textarea name="remark" id="remark" class="form-control addressfield" required></textarea>
              </div>
            </div>
          </div>
        <div class="form-group">
            <div class="col-md-12" align="center">
              <input type="submit" class="btn<?=$btncolor?>" name="upd" id="upd" value="Save" title="Approve Payment">
			  <input type="hidden" name="adjust_type" id="adjust_type" value="<?=$p_row['adjustment_type']?>">
			  <input type="hidden"  id="partycode" name="partycode" value="<?=$p_row['to_location']?>" >
			  <input type="hidden"  id="parentcode" name="parentcode" value="<?=$p_row['from_location']?>" >
			  <input type="hidden"  id="pay_date" name="pay_date" value="<?=$p_row['payment_date']?>" >
               <input title="Back" type="button" class="btn<?=$btncolor?>" value="Back" onClick="window.location.href='paymentsend_approval_list.php?<?=$pagenav?>'">
            </div>
          </div>
         </form>
