...
////// we hit save button
 ...
 if (isset($_POST['upd']) && $_POST['upd']=='Save'){
	...
	$res_po=mysqli_query($link1,"select max(crdr_counter) as no, financial_year,doc_code from document_counter where location_code='".$parentcode."'");
	if(mysqli_num_rows($res_po)){
	$row_po=mysqli_fetch_array($res_po);
	$c_nos=$row_po['no']+1;
	$pad1=str_pad($c_nos,4,"0",STR_PAD_LEFT);  
	$doc_no=$_POST['adjust_type']."/".$row_po['doc_code']."/".$row_po['financial_year']."/".$pad1; 
	///// update counter //////////////////////////////////
    mysqli_query($link1,"update document_counter set crdr_counter = '".$c_nos."'  where  location_code='".$parentcode."' "	);
	
	mysqli_autocommit($link1, false);
	$flag = true;
	///// Insert Master Data
   $query1= "INSERT INTO payment_receive set doc_no='".$doc_no."',from_location='".$partycode."',to_location='".$parentcode."',amount='".$amount."',status='PFA',payment_mode='".$pay_mode."', bank_name='".$bank_name."', bank_branch='".$bank_branch."', dd_cheque_no='".$dd_ch_no."', dd_cheque_dt='".$dd_ch_dt."', receipt_no='".$rec_no."', transaction_id='".$trans_id."', remark='".$remark."',entry_dt='".$today."',entry_time='".$currtime."',entry_by='".$_SESSION['userid']."',ip='".$ip."' , payment_date = '".$payment_date."' , adjustment_type = '".$adjust_type."'  ";
	...
	$flag=dailyActivity($_SESSION['userid'],$doc_no,"Account Adjustment","Account Adjustment",$ip,$link1,$flag);
	///// check both master and data query are successfully executed
	if ($flag) {
        mysqli_commit($link1);
        $msg = "Account Adjust successfully  with ref. no.".$doc_no;
    } else {
	...
	///// move to parent page
 header("location:acc_adjustList.php?msg=".$msg."".$pagenav);
   exit;
 }
...
{var val=document.getElementById('pay_mode').value;
	if(val==1 || val == 6 || val == 7 || val == 8 || val == 9 || val == 10 || val == 11)
	{
...
<div class="container-fluid">
  <div class="row content">
    ...
    ?>
    <div class="col-sm-9">
      <h2 align="center"><i class="fa fa-credit-card-alt"></i> Account Adjust </h2>
      ...
      <div class="form-group"  id="page-wrap" style="margin-left:10px;">
          ...
          <div class="form-group">
            <div class="col-md-10"><label class="col-md-3 control-label">To Location<span style="color:#F00">*</span></label>
              <div class="col-md-9">
                  ...
                  <select name="from_location" id="from_location"  class="form-control selectpicker required" data-live-search="true" onChange="document.frm1.submit();" required>
          ...
          <div class="form-group">
            <div class="col-md-10"><label class="col-md-3 control-label">From Location<span style="color:#F00">*</span></label>
              <div class="col-md-9">
                 <select name="to_location" id="to_location"  class="form-control selectpicker required" data-live-search="true" onChange="document.frm1.submit();" required>
                 <option value="" selected="selected">Please Select </option>
                    <?php 
          ...
          <div class="col-md-10">
          <div class="col-md-6">
              <label class="col-md-6 control-label">Adjustment Type<span style="color:#F00">*</span></label>
              <div class="col-md-6">
             <select name="adjust_type" id="adjust_type"  class="form-control required" data-live-search="true"  required>
        ...
        <div class="form-group">
          ...
          <div class="col-md-6">
              <label class="col-md-6 id="pay_date" control-label">Payment Date<span style="color:#F00">*</span></label>
              <div class="col-md-6">
           <input type="text" class="form-control span2" name="payment_date"  id="payment_date"  value="<?php if(isset($_REQUEST['payment_date'])){echo $_REQUEST['payment_date']; } else{echo $today;}?>" required>
              </div>
              </div>
          </div>
        </div>
        ...
        <div class="form-group">
            ...
            <input title="Back" type="button" class="btn<?=$btncolor?>" value="Back" onClick="window.location.href='acc_adjustList.php?<?=$pagenav?>'">
            </div>
          </div>
         </form>
