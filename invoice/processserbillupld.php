...
if($_POST['upd']=="Process"){
			...
			while($row2 = mysqli_fetch_assoc($res2)){
				...
				if($row2['prod_code']!='' && $row1['bill_to']!='' && $row2['qty']!='' && $row2['qty']!=0) {
					...
					while($row3 = mysqli_fetch_assoc($res3)){
						///// check serial no. should not repeat again
						$res_chkserial = mysqli_query($link1,"SELECT owner_code FROM billing_imei_data WHERE imei1='".$row3['imei1']."' ORDER BY id DESC");
						$row_chkserial = mysqli_fetch_assoc($res_chkserial);
						$num_chkserial = mysqli_num_rows($res_chkserial);
						if($row_chkserial["owner_code"]==$row3['bill_from'] || $num_chkserial==0){
							...
							$billed_ser[] = $row3['imei1'];
							$flag = false;
							$err_msg = "Error Code5: Serial no. are already billed";
						}
					}
					$basecost+=$value;
					$taxcost+=$taxamt;
	...
	if ($flag) {
		...
		if($dupli_inv){
			...
			$msg = "Request could not be processed ".$err_msg." ".implode(",",$billed_ser);
		}else{
