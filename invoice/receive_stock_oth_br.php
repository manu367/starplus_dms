	...
	if ($_POST['updpod']=='Upload'){
		...
		if($_FILES['pod1']['name']){
			...
			if (!in_array($extpod1, $allowed)) {
				...
				header("location:receive_stock_oth_br.php?msg=".$msg."".$pagenav);
				exit;
			}else{
...
<div class="container-fluid">
  <div class="row content">
    ...
    <div class="col-sm-9 tab-pane fade in active table-responsive" id="home">
      <h2 align="center"><i class="fa fa-level-down"></i> Receive Stock List(Distributor/ASD/SSD/Shoppe)</h2>
      ...
      <th width="6%" data-hide="phone,tablet">Receive Stock <br>By Scanning</th>
          ...
          <tbody>
			...
			if($_SESSION['userid']=="admin"){
				//$sql=mysqli_query($link1,"Select * from billing_master where status in ('Dispatched','Received') AND type NOT IN ('GRN','LP','DIRECT SALE RETURN') AND (to_location LIKE 'CART%' OR to_location LIKE 'CADS%' OR to_location LIKE 'CASR%')  AND entry_date>='2023-01-20' ".$filter_str." order by id desc");
				$sql=mysqli_query($link1,"Select * from billing_master where status in ('Dispatched','Received') AND type NOT IN ('GRN','LP','DIRECT SALE RETURN') AND (to_location LIKE 'SURT%' OR to_location LIKE 'SUDS%' OR to_location LIKE 'SUSR%')  AND entry_date>='2023-01-20' ".$filter_str." order by id desc");
			}else{
				
				$sql=mysqli_query($link1,"Select * from billing_master where to_location in (".$accesslocation.") and status in ('Dispatched','Received') AND type NOT IN ('GRN','LP','DIRECT SALE RETURN') AND (to_location LIKE 'SURT%' OR to_location LIKE 'SUDS%' OR to_location LIKE 'SUSR%')  AND entry_date>='2023-01-20' ".$filter_str." order by id desc");
			}
            ...
            <tr class="even pointer">
              ...
              <td align="left"><a href='receiveStockOthBrView.php?rb=view&id=<?php echo base64_encode($row['challan_no']);?><?=$pagenav?>'  title='view'><i class="fa fa-eye fa-lg" title="view details"></i></a></td>
              <td align="left"><?php 
			  if($check==1){
			  ...
			  ?>
              <a href='receiveStockOthBrActionPRN.php?rb=view&id=<?php echo base64_encode($row['challan_no']);?><?=$pagenav?>'  title='Receive Invoice'><i class="fa fa-shopping-bag fa-lg" title="Receive Invoice"></i></a>
			  <?php }else{
			  ?><a href='receiveStockOthBrAction.php?rb=view&id=<?php echo base64_encode($row['challan_no']);?><?=$pagenav?>'  title='Receive Invoice'><i class="fa fa-shopping-bag fa-lg" title="Receive Invoice"></i></a><?php }}}else{ echo "<b style='color:#F30'>Please Upload Serial No. for Serialized Product</b>";}?></td>
				
<td align="left"><?php 
if($check==1){
if($row['status']=="Dispatched"){?><?php /*?><a href='receiveStockOthBrByScanning.php?rb=view&id=<?php echo base64_encode($row['challan_no']);?><?=$pagenav?>'  title='Serial No.Scan'><i class="fa fa-qrcode fa-lg"></i></a><?php */?><?php }}else{ echo "<b style='color:#F30'>Please Upload Serial No. for Serialized Product</b>";} ?></td>
            <?php }?>
          </tbody>
          </table>
      </div>
