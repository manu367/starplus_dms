...
<div class="container-fluid">
  <div class="row content">
    ...
    <div class="col-sm-9 tab-pane fade in active table-responsive" id="home">
      <h2 align="center"><i class="fa fa-user-circle-o"></i> Sale Invoice Against PO</h2>
      ...
      <div class="form-group"  id="page-wrap" style="margin-left:10px;"><br/><br/>
       <table  width="98%" id="myTable" class="table-striped table-bordered table-hover" align="center">
          ...
          <tbody>
			...
			if($_SESSION["userid"]=="admin"){
				$sql=mysqli_query($link1,"Select * from purchase_order_master where 1 AND req_type NOT IN ('COMBO PO') ".$filter_str." ".$statt." AND status='Approved' order by id desc");
			}else{
				$sql=mysqli_query($link1,"Select * from purchase_order_master where po_from in (".$accesslocation.") AND req_type NOT IN ('COMBO PO') AND status='Approved' AND po_no IN (SELECT po_no FROM purchase_order_data WHERE psc_id IN (".$accesspsc.") AND brand_id IN (".$accessbrand.")) ".$filter_str." ".$statt." order by id desc");
			}
            ...
            <tr class="even pointer">
              ...
              <td align="center"><a href='poDetails.php?id=<?php echo base64_encode($row['po_no']);?><?=$pagenav?>'  title='view'><i class="fa fa-eye fa-lg" title="view details"></i></a></td>
              <td align="center"><?php if($row["sale_type"]=="PRIMARY" || substr($row['po_from'],0,4)=="EASR" || substr($row['po_from'],0,4)=="EART" || substr($row['po_from'],0,4)=="EABR"){?>
              <a href='#' onClick="openPoAllocation('<?=base64_encode($row['po_no'])?>')"><i class='fa fa-external-link fa-lg' title='PO Allocation'></i></a>&nbsp;&nbsp;
              <?php } ?>
              <a href='invoiceAgainstPON_ADV.php?id=<?php echo base64_encode($row['po_no']);?><?=$pagenav?>'  title='Go to Advance invoicing'><i class="fa fa-envelope-open fa-lg" title="Go to Advance invoicing"></i></a></td>
            </tr>
            <?php }?>
          </tbody>
          </table>
