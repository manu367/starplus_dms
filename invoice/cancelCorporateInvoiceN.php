...
if($_POST){
  if($_POST['Submit']=='Cancel'){
	  ...
	  while($vpo_row=mysqli_fetch_assoc($vpo_res)){
	     ...
	     $result=mysqli_query($link1, "update stock_status set okqty=okqty+'".$vpo_row['qty']."',updatedate='".$datetime."' where asc_code='".$inv_row['from_location']."' and sub_location='".$inv_row['sub_location']."' and partcode='".$vpo_row['prod_code']."'");
		//// check if query is not executed
		if (!$result) {
		   $flag = false;
		 ...
		 $flag=stockLedger($docid,$today,$vpo_row['prod_code'],$inv_row['sub_location'],$inv_row['to_location'],$inv_row['sub_location'],"IN","OK","Cancel Corporate Invoice",$vpo_row['qty'],$vpo_row['price'],$_SESSION['userid'],$today,$currtime,$ip,$link1,$flag);
		 }	 
	  }/// close for loop
