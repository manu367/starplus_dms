...
if($_POST) {
	if($_POST['upd'] == 'Process') {
		...
		$sessionMessageIdent = isset($_SESSION['msgiapo'])?$_SESSION['msgiapo']:'';
		if($messageIdent!=$sessionMessageIdent){//if its different:
			//save the session var:
			$_SESSION['msgiapo'] = $messageIdent;
    		if($total_qty != '' && $total_qty != 0) {
						...
						if(is_array($_POST['prod_code'])){
							...
							if($len>0){
								for($i=0;$i<$len;$i++){			
									...
									if($addbill_qty[$i]!="" && $addbill_qty[$i]!="0"){					
				...
				$msg = "Request could not be processed . Invoice is already generated against this PO .".$po_row['po_no'];
			}
			}else{
...
/////// used for discount calculation ///////	
	...
	document.getElementById("doctype").value = "DC";
	  ...
	  location.reload();
			...
			$(document).ready(function() {
                $("#add_row").click(function() {
                   var ni = document.getElementById('newpartrow');
					...
					//newdiv.innerHTML = '<tr id="addr'+num+'"><td><input name="sno[' + num + ']"  id="sno[' + num + ']" type="text" class="form-control" value="' + nextnum + '" size="3" readonly style="width:35px" /></td><td><select class="form-control selectpicker" data-live-search="true" name="prod_code[' + num + ']" id="prod_code[' + num + ']" required  onChange="getAvlStk(' + num + ');"><option value="">--None--</option><?php $model_query = "select productcode,productname,productcolor from product_master where status='Active'";$check1 = mysqli_query($link1, $model_query);while ($br = mysqli_fetch_array($check1)) {?><option value="<?php echo $br['productcode']; ?>"><?php echo $br['productname']." | ".$br['productcode']." | ".$br['productcolor']; ?></option><?php } ?></select></td><td><input name="qty[' + num + ']" id="qty[' + num + ']" type="text" class="number form-control required" size="10"  onBlur="fillCost_newadded('+num+');" style="width:80px;text-align:right;" value="0"/><input name="uom['+ num +']" id="uom['+ num +']" type="text" style="width:50px;" class="form-control"/><span id="err_qty['+num+']" class="red_small"></span></td><td><input name="box_qty['+num+']" id="box_qty['+num+']" type="text" class="number form-control" size="10" style="width:70px;text-align:right;display:none;"/><input name="checkBoxQty['+num+']" id="checkBoxQty['+num+']" type="hidden" value="0"/></td><td><input name="po_qty['+num+']" id="po_qty['+num+']" type="text" class="form-control" style"width:80px; text-align:right;" size="10" value="0" readonly/></td><td><input name="abl_qty['+num+']" id="abl_qty['+num+']" type="text" class="form-control" style="width:100px; text-align:right;" size="10" value="" readonly/></td><td><input name="rate['+num+']" id="rate['+num+']" type="text" class="number form-control required" style="width:100px; text-align:right;" size="10" value="" onkeyup="fillCost_newadded('+num+');"/></td><td><input name="total['+num+']" id="total['+num+']" type="text"  class="form-control" style="width:100px; text-align:right;" size="10" value="0.00" readonly/><input name="holdRate['+num+']" id="holdRate['+num+']" type="hidden"/><input name="prod_cat['+num+']" id="prod_cat['+num+']" type="hidden"/><div style="display:inline-block;float:right"><i class="fa fa-close fa-lg" onClick="deleteRow(' + num + ');"></i></div></td></tr>';
            ...
            /*function  fillCost_newadded(ind){
				  var ent_qty="bill_qty["+ind+"]";
				  var availableQty="avl_stock["+ind+"]";
				  var ent_rate="price["+ind+"]";
				  var discountField = "rowdiscount" + "[" + ind + "]";
				  ...
				  var rowcgstper = "cgst_per" + "[" + ind + "]";
					...
					<?php }?>
				  var qty=document.getElementById(ent_qty).value;
				  var ablqty=document.getElementById(availableQty).value;
				  var price=document.getElementById(ent_rate).value;
				  var rdiscount=document.getElementById(discountField).value;
				  var rvad=document.getElementById(rst).value;
				  var rsgstamt=document.getElementById(rowsgstamount).value;
				  var rcgstamt=document.getElementById(rowcgstamount).value;
				  var rigstamt=document.getElementById(rowigstamount).value;
				  var rtotval=document.getElementById(totalvalField).value;
				  if((ablqty) >= (qty)){
					  var total= parseFloat(qty)*parseFloat(price);
					  var var3="value["+ind+"]";
					  ...
					  document.getElementById(var3).value=(total);
					  document.getElementById(var3).value=(total);
					  document.getElementById("err_qty["+ind+"]").innerHTML = "";
					  calculatetotal_addnew();
				  }else{
					  document.getElementById("err_qty["+ind+"]").innerHTML = "Stock is not Available";
					  document.getElementById(ent_qty).value="0";
					  var total= parseFloat(document.getElementById(ent_qty).value)*parseFloat(price);
					  var var3="value["+ind+"]";
					  document.getElementById(var3).value=(total);
					  calculatetotal_addnew();
				  }
    ...
    <body onKeyPress="return keyPressed(event);" onLoad="recount()">
        <div class="container-fluid">
            <div class="row content">
                ...
                ?>
                <div class="col-sm-9">
                    <h2 align="center"><i class="fa fa-user-circle-o"></i> Sale Invoicing against PO (Advance)</h2><br/>
                            ...
                            <div class="panel panel-info table-responsive">
                                ...
                                <div class="panel-body">
                                    ...
                                    <tfoot id='productfooter' style="z-index:-9999;">
                                            <tr class="0">
                                                <td colspan="12" style="font-size:13px;"><a id="add_row" style="text-decoration:none"><i class="fa fa-plus-square-o fa-2x"></i>&nbsp;Add Row</a><input type="hidden" name="theValue" id="theValue" value="-1"/></td>
                                            </tr>
                                    	</tfoot>
                                </div><!--close panel body-->
                            </div><!--close panel-->
                                    ...
                                    <table class="table table-bordered" width="100%">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td width="30%"><input type="text" name="total_qty" id="total_qty" class="form-control" value="<?=$totqty;?>" readonly style="width:200px;"/><input type="hidden" name="total_qty_ext" id="total_qty_ext" class="form-control" readonly value="0"/><input type="hidden" name="total_qty_add" id="total_qty_add" class="form-control" value="0" readonly style="width:200px;"/> <input type="hidden" name="total_qty1" id="total_qty1" value="0" /> </td>
                                                <td width="20%"><label class="control-label">Sub Total</label></td>
                                                <td width="30%"><input type="text" name="sub_total" id="sub_total" class="form-control" value="<?= currencyFormat($subtotal) ?>" readonly style="width:200px;text-align:right"/><input type="hidden" name="sub_total_add" id="sub_total_add" value="0.00"/><input type="hidden" name="sub_total_ext" id="sub_total_ext" value="0.00"/></td>
                                            </tr>
                                            <tr>
                                                ...
                                                <td><input type="text" name="total_discount" id="total_discount" class="form-control" value="<?=$totdiscount?>" style="width:200px;text-align:right" onKeyPress="return onlyFloatNum(this.value);" onKeyUp="check_total_tax();" readonly/><input type="hidden" name="total_discount_add" id="total_discount_add" value="0.00"/><input type="hidden" name="total_discount_ext" id="total_discount_ext" value="0.00"/></td>
                                            </tr>
                                            <tr>
                                              <td><label class="control-label">Total SGST</label></td>
                                              <td><input type="text" name="total_sgst" id="total_sgst" class="form-control" value="<?= $totsgst ?>" readonly style="width:200px;text-align:right"/><input type="hidden" name="total_sgst_add" id="total_sgst_add" value="0.00"/><input type="hidden" name="total_sgst_ext" id="total_sgst_ext" value="0.00"/></td>
                                            </tr>
                                            <tr>
                                              <td height="56"><label class="control-label">Ship To</label></td>
                                              <td><select name="ship_to" id="ship_to" class="form-control selectpicker" data-live-search="true" style="width:200px;" onChange="getShipToInfo(this.value);">
                                            <option value="" selected="selected">Please Select </option>
                                            <?php
                                            ...
                                            ?>
                                              ...
                                              <td><input type="text" name="total_cgst" id="total_cgst" class="form-control" value="<?= $totcgst ?>" readonly style="width:200px;text-align:right"/><input type="hidden" name="total_cgst_add" id="total_cgst_add" value="0.00"/><input type="hidden" name="total_cgst_ext" id="total_cgst_ext" value="0.00"/></td>
                                            </tr>
                                            <tr>
                                              ...
                                              <td><input type="text" name="total_igst" id="total_igst" class="form-control" value="<?= $totigst ?>" readonly style="width:200px;text-align:right"/><input type="hidden" name="total_igst_add" id="total_igst_add" value="0.00"/><input type="hidden" name="total_igst_ext" id="total_igst_ext" value="0.00"/></td>
                                            </tr>
                                            <tr>
                                              ...
                                              <td><input type="text" name="shipto_city" id="shipto_city" class="form-control mastername" style="width:200px;"></td>
                                                ...
                                                <td><input type="text" name="grand_total" id="grand_total" class="form-control" value="<?php echo currencyFormat($grandtot); ?>" readonly style="width:200px;text-align:right"/><input type="hidden" name="grand_total_add" id="grand_total_add" value="0.00"/><input type="hidden" name="grand_total_ext" id="grand_total_ext" value="0.00"/></td>
                                            </tr>
                                            <tr>
                   ...
                   <tr>
                     ...
                     <td><textarea name="remark" id="remark" class="form-control" style="resize:none;width:200px" rows="5" ></textarea></td>
                               ...
                               <?php if($_REQUEST['doc_type']=="DC"){ if($_REQUEST['ledger_name']==""){ $btn_enable=0;}else{$btn_enable=1;}}else{$btn_enable=1;} ?>
                                            <tr>
                                                <td colspan="4" align="center"><input type="submit" class="btn <?=$btncolor?>" name="upd" id="upd" value="Process" title="Make Invoice" <?php if($btn_enable==0 || $_POST["upd"]=="Process"){?> disabled<?php }?>>
