...
if ($_POST['Submit']=="Upload"){
	if ($_FILES["attchfile"]["name"]) {
	 		...
	 		if ($k == 0 || $k == 1) continue; // skip first row 
	  		for( $i = 0; $i < count($k); $i++)
	  		{
	      		...
	      		if($r[0]=="EOF"){
					...
					if($row_imei['owner_code']==$biillingFrom && $row_imei["prod_code"]==$partcode){
						...
						$ser_notinstk[] = $serialno;
	      				$flag = false;
						$err_msg = "Serial nos not in stock.";
					}
		  		}
	  		}
		}//Close For loop
		...
		if(count($ser_dup1)!=$total_ser){
			...
			header("location:uploadBillingSerialDS.php?".$pagenav);
			exit;
		}
		if ($flag) {
			mysqli_commit($link1);
		...
		if($ser_notinstk){
			$msg = "Serial nos not in stock.";
			///// move to parent page
			$_SESSION["logres"] = [ "status"=>"failed", "msg"=> $msg, "invalid"=>$ser_notinstk];
			header("location:uploadBillingSerialDS.php?".$pagenav);
			exit;
		}else if($arr_invalid){
			$upd_cnt = 0;
			...
			if($eof=='1'){
				...
				header("Location:showtempdsserbilldata.php?msg=sucess&f_name=".$f_name."&file_id=".$file_id."&fromloc=".$biillingFrom."&rmk=".base64_encode($remark)."".$pagenav);
				exit;
			}
			else{
...
<div class="container-fluid">
    ...
    <div class="col-sm-9 tab-pane fade in active" id="home">
    	<h2 align="center"><i class="fa fa-upload"></i> Upload Billing Serialwise (DS)</h2>
...
<a href="../admin/excelexport.php?rname=<?=base64_encode("locationmaster")?>&rheader=<?=base64_encode("Location Master")?>&locstate=<?=base64_encode($_REQUEST['state'])?>&loccity=<?=base64_encode($_REQUEST['city'])?>&loctype=<?=base64_encode("DL")?>&locstatus=<?=base64_encode($_REQUEST['status'])?>" title="Export Dealer details in excel"><i class="fa fa-file-excel-o fa-2x" title="Export Dealer details in excel"></i></a>Download Dealer master list
<br/>
<a href="../admin/excelexport.php?rname=<?=base64_encode("locationmaster")?>&rheader=<?=base64_encode("Location Master")?>&locstate=<?=base64_encode($_REQUEST['state'])?>&loccity=<?=base64_encode($_REQUEST['city'])?>&loctype=<?=base64_encode("RT")?>&locstatus=<?=base64_encode($_REQUEST['status'])?>" title="Export Retailer details in excel"><i class="fa fa-file-excel-o fa-2x" title="Export Retailer details in excel"></i></a>Download Retailer master list        
        </div>
            	...
            	<div class="col-md-12"><label class="col-md-4 control-label">From Location <span class="red_small">*</span></label>
              		<div class="col-md-4">
              			<select name="biillingFrom" id="biillingFrom" required class="form-control selectpicker required" data-live-search="true" onChange="document.frm1.submit();">
							...
							<?php 
                            $sql_parent = "SELECT * FROM access_location WHERE uid='".$_SESSION['userid']."' AND status='Y' AND id_type IN ('DS')";
