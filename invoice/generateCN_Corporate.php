...
if($_POST) {
	...
	$sessionMessageIdent = isset($_SESSION['msgCorpSRN'])?$_SESSION['msgCorpSRN']:'';
	if($messageIdent!=$sessionMessageIdent){//if its different:
		//save the session var:
		$_SESSION['msgCorpSRN'] = $messageIdent;
		///start transaction
        ...
        while($vpo_row = mysqli_fetch_assoc($vpo_res)) {
			...
			if($vpo_row["prod_cat"]!="C"){
				...
				$flag = stockLedger($docid, $today, $vpo_row['prod_code'], $po_row['sub_location'], $po_row['to_location'], $po_row['sub_location'], "IN", "OK", "Cancel Corporate Invoice", $vpo_row['qty'], $vpo_row['price'], $_SESSION['userid'], $today, $currtime, $ip, $link1, $flag);
				$i++;
			}
			if($po_row["document_type"]=="INVOICE"){
				/////// credit note data
       ...
       /* ///// check retail billing was not for customer. we reverse credit balance for location only not for customer
        ...
        mysqli_close($link1);
        ///// move to parent page
        header("Location:corporateInvoiceList.php?msg=" . $msg . "&chkflag=" . $cflag . "&chkmsg=" . $cmsg . "" . $pagenav);
        exit;
    }
}
?>
