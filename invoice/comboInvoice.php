...
$_SESSION['msgiacpo']="";
///// if action taken
if($_POST['updtoloc']=="Update"){
	@extract($_POST);
	$sql_doc = "INSERT INTO po_allocation_history SET ref_no='".base64_decode($ref_no)."', old_location='".base64_decode($old_loc)."', new_location='".$po_allocate_to."', entry_by='".$_SESSION["userid"]."', entry_date='".$datetime."', entry_ip='".$ip."', remark='".$poallocrmk."'";
	...
	if (!$res_doc) {
		...
		///// check if approved
		mysqli_query($link1,"UPDATE purchase_order_master SET po_to='".$po_allocate_to."' WHERE po_no = '".base64_decode($ref_no)."'");
		$cflag = "success";
		$cmsg = "Success";
		$msg = "PO ".base64_decode($ref_no)." is now allocate to ".$po_allocate_to;
	}
	header("location:corporateInvoice.php?msg=".$msg."&chkflag=".$cflag."&chkmsg=".$cmsg."".$pagenav);
	exit;
}
///// get access location ///
...
////// function for open model to PO Allocation
function openPoAllocation(docid){
	$.get('po_allocation.php?doc_id=' + docid, function(html){
...
<div class="container-fluid">
  <div class="row content">
    ...
    <div class="col-sm-9 tab-pane fade in active table-responsive" id="home">
      <h2 align="center"><i class="fa fa-cubes"></i> Sale Invoice Against Combo PO</h2>
      ...
      <form class="form-horizontal" role="form" name="frm1" action="" method="POST">
          <div class="row">
            ...
            <?php /*?><div class="col-sm-3 col-md-3 col-lg-3"><label class="col-md-6">Status</label>
      ...
      <form class="form-horizontal" role="form">
        <?php /*?><div style="display:inline-block;float:right">
        <button title="Upload Billing" type="button" class="btn <?=$btncolor?>" style="float:right;" onClick="window.location.href='uploadBilling.php?<?=$pagenav?>'"><span>Upload Billing</span></button>&nbsp;&nbsp;&nbsp;&nbsp;</div><?php */?>
        <div style="display:inline-block;float:right">
        <button title="Invoice list" type="button" class="btn <?=$btncolor?>" style="float:right;" onClick="window.location.href='corporateInvoiceList.php?<?=$pagenav?>'"><span>Invoice list</span></button></div>
       ...
       <table  width="98%" id="myTable" class="table-striped table-bordered table-hover" align="center">
          <thead>
            <tr class="<?=$tableheadcolor?>">
              ...
              <th data-hide="phone,tablet">PO View</th>
              <th data-hide="phone,tablet">Invoicing</th>
            </tr>
          </thead>
          <tbody>
            <?php
			...
			///// get access location ///
			//$accesslocation=getAccessLocation($_SESSION['userid'],$link1);
			//$sql=mysqli_query($link1,"Select * from purchase_order_master where po_from in (".$accesslocation.") and status='Approved' order by id desc");
			if($_SESSION["userid"]=="admin"){
				$sql=mysqli_query($link1,"Select * from purchase_order_master where 1 AND req_type IN ('COMBO PO') ".$filter_str." ".$statt." AND status='Approved' order by id desc");
			}else{
				 //  remove  brand and  psc_id  condition - ravi 02-03-2023
				//$sql=mysqli_query($link1,"Select * from purchase_order_master where po_from in (".$accesslocation.") AND req_type IN ('COMBO PO') AND status='Approved' AND po_no IN (SELECT po_no FROM purchase_order_data WHERE psc_id IN (".$accesspsc.") AND brand_id IN (".$accessbrand.")) ".$filter_str." ".$statt." order by id desc");
				$sql=mysqli_query($link1,"Select * from purchase_order_master where po_from in (".$accesslocation.") AND req_type IN ('COMBO PO') AND status='Approved'  ".$filter_str." ".$statt." order by id desc");
			}
            ...
            <tr class="even pointer">
              ...
              <td align="center"><a href='comboPoDetails.php?id=<?php echo base64_encode($row['po_no']);?><?=$pagenav?>'  title='view'><i class="fa fa-eye fa-lg" title="view details"></i></a></td>
              <td align="center"><?php if($row["sale_type"]=="PRIMARY" || substr($row['po_from'],0,4)=="EASR"){?>
              <a href='#' onClick="openPoAllocation('<?=base64_encode($row['po_no'])?>')"><i class='fa fa-external-link fa-lg' title='PO Allocation'></i></a>&nbsp;&nbsp;
              <?php } ?>
              <a href='invoiceAgainstCPON_ADV.php?id=<?php echo base64_encode($row['po_no']);?><?=$pagenav?>'  title='Go to Advance invoicing'><i class="fa fa-envelope-open fa-lg" title="Go to Advance invoicing"></i></a></td>
            </tr>
            <?php }?>
          </tbody>
          </table>
    ...
    <div class="modal-dialog modal-dialogTH modal-lg">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title" align="center"><i class='fa fa-external-link fa-lg faicon'></i> PO Allocation</h4>
        </div>
        <div class="modal-body modal-bodyTH">
         <!-- here dynamic task details will show -->
        </div>
        <div class="modal-footer">
          <input type="submit" class="btn<?=$btncolor?>" name="updtoloc" id="updtoloc" value="Update" title="" <?php if($_POST['updtoloc']=='Update'){?>disabled<?php }?>>
