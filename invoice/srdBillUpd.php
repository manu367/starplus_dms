...
if($_POST['Submit']=="Upload"){
   if ($_FILES["attchfile"]["name"]) {
	...
	foreach( $xlsx->rows() as $k => $r) {
	 if ($k == 0 || $k == 1) continue; // skip first row 
	  for( $i = 0; $i < count($k); $i++)
	  {
	      ...
	      $sql="INSERT INTO temp_bill_upload set bill_from='".$biillingFrom."',bill_to='".$biillingTo."',prod_code='".$partcode."',imei1=".$imei1.",imei2='".$imei2."',bill_date='".$billdate."',invoiceno='".$invoiceno."',invoicedate='".$invoicedate."',price='".$price."',update_by='".$_SESSION['userid']."',browserid='".$browserid."',file_id='".$file_id."'";
	...
	if($eof=='1'){
		...
		header("Location:showtempSRDbill.php?msg=sucess&f_name=$f_name&file_id=$file_id&fromloc=$biillingFrom&toloc=$biillingTo&bdate=$billdate&discount=$discount".$pagenav);
		exit;
    }
	else{
...
<div class="container-fluid">
  <div class="row content">
    ...
    <div class="col-sm-9 tab-pane fade in active" id="home">
      <h2 align="center"><i class="fa fa-upload"></i> Upload Billing</h2><div style="display:inline-block;float:right"><a href="../templates/UPLOAD_SRDBILL.xlsx" title="Download Excel Template"><img src="../img/template.png" title="Download Excel Template"/></a></div><br/>
      ...
      <?php }?>
        <form  name="frm1"  id="frm1" class="form-horizontal" action="" method="post"  enctype="multipart/form-data">
          ...
          <!--<div class="form-group">
            <div class="col-md-12"><label class="col-md-4 control-label">Discount</label>
              <div class="col-md-4">
              <input type="text" name="discount" id="discount" class="form-control" value="0.00" style="width:300px;text-align:right" onkeypress="return onlyFloatNum(this.value);"/>
              </div>
            </div>
          </div>-->
          <div class="form-group">
...
</html>Browse&hellip; <input type="file" name="attchfile" class="form-control required" required style="display:none;" accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet">
                        </span>
                    </label>
                    <input type="text" class="form-control" name="billfile"  id="billfile" readonly>
                </div>
