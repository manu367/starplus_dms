		...
		if($_FILES['doc_attachment']['name']){
	  	...
	  	if($_POST['actiontaken'] == 'Approved'){
			...
			$gst_type = "";
	   		...
	   		$data = mysqli_query($link1 , "SELECT * from billing_model_data where challan_no = '".$decodepono."' ");
	   		while($row = mysqli_fetch_array($data)){
	    		$qtycheck = mysqli_query($link1 , "select okqty from stock_status where partcode = '".$row['prod_code']."' and asc_code = '".$row['from_location']."' and sub_location='".$po_row["sub_location"]."'  and okqty >='".$row['qty']."'");
				if(mysqli_num_rows($qtycheck)>0 || $row['qty']=="0.00") {
					if($row['sgst_per']!="" && $row['cgst_per']!="0.00"  && $row['sgst_per']!="0"){
						$gstper = $row['sgst_per'] + $row['cgst_per'];					
						$arr_taxx[$gstper] += $row['sgst_amt'] + $row['cgst_amt'];
						$arr_val[$gstper] += $row['value'];
						$gst_type = "SGST-CGST";
					}else{
						$gstper = $row['igst_per'];					
						$arr_taxx[$gstper] += $row['igst_amt'];
						$arr_val[$gstper] += $row['value'];
						$gst_type = "IGST";
					}
	    			$check = mysqli_query($link1, " update stock_status set hold_okqty=hold_okqty-'".$row['qty']."', okqty = okqty-'".$row['qty']."' where partcode = '".$row['prod_code']."' and asc_code = '".$row['from_location']."' and sub_location='".$po_row["sub_location"]."' ");	      
					...
					if (!$check) {
	   				...
	   				$flag=stockLedger($decodepono,$today,$row['prod_code'], $po_row["sub_location"],$tolocation,$po_row["sub_location"],"OUT","OK","Stock Transfer",$row['qty'],$row['price'],$_SESSION['userid'],$today,$currtime,$ip,$link1,$flag); 
				}          
				else {
	      			$msg="Stock is not Available to transfer".$row['prod_code'];
	  				 ///// move to parent page
            		header("Location:stockTRansferApproval.php?msg=".$msg."".$pagenav);
            		exit;
				}
			...
			/////// make account ledger entry for location
				/////// start ledger entry for tally purpose ///// written by shekhar on 23 dec 2022
				///// make ledger array which are need to be process
				/*$arr_ldg_name = array(
				"igstldgname" => "IGST @",
				"cgstldgname" => "CGST @",
				"sgstldgname" => "SGST @",
				"igstdocldgname" => "Branch Transfer @",
				"cgstdocldgname" => "Branch Transfer within State @",
				"sgstdocldgname" => "Branch Transfer within State @",
				...
				$resp = explode("~",storeLedgerTransaction($fromloction,$decodepono,$today,$hedid,$hed,$arr_taxx,$arr_val,$tcs_per,$tcs_amt,$round_off,$gst_type,$arr_ldg_name,"Stock Transfer Sales","Sales Accounts",$link1,$flag));
				$flag = $resp[0];
				if($err_msg==""){
					$err_msg = $resp[1];
				}
				/////// end ledger entry for tally purpose ///// written by shekhar on 23 dec 2022
	 		////// return message
