...
while($row=mysqli_fetch_array($rs)){
	$comboprodlist_res = mysqli_query($link1,"SELECT GROUP_CONCAT(bom_partcode) AS pc, GROUP_CONCAT(bom_qty) AS qt FROM combo_master WHERE bom_modelcode='".$row['prod_code']."' AND  status='1'");
	...
	for($k=0; $k<count($expl_prod); $k++){
		$product_name = explode("~", getProductDetails($expl_prod[$k], "productname,model_name", $link1));
		if($prd_str){
	...
	$combo_det = getAnyDetails($row['prod_code'],"bom_modelname","bom_modelcode","combo_master",$link1);
?>
                    <tr>
                      <td align=center><span class="style6"><strong><?=$i?></strong></span></td>
                      <td align=left><span class="style6"><strong><?=$combo_det." (".$row['prod_code'].")<br/><i>".$prd_str."</i>";?></strong></span></td>
