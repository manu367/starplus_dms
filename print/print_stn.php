...
$to_statecode = getAnyDetails($to[3],"statecode","state","state_master",$link1);
$from_statecode = getAnyDetails($loc[3],"statecode","state","state_master",$link1);
...
//-------------Getting invoice details from billing data---------------------//
                                           ...
                                           <!-- <tr>
											...
											<?php	if($loc[3]==$to[3]){ $colspn1=6; }else{ $colspn1=6;} ?>	
											<tr>
											<?php	if($loc[3]==$to[3]){ $colspn2=3; }else{ $colspn2=3;} ?>	
                               <td colspan="<?=$colspn2?>"><B>Total Weight</B></td>
							   
                                                <td align="left"  colspan="<?=$colspn1?>"><B>Total Quantity:</B><?php echo currencyFormat($total); ?></td>
                                                
												<td align="right" colspan="<?=$colspn1?>"><B>Total Amount(INR):<?php echo currencyFormat($value); ?></B></td>
