...
if (!$data || empty($data['email']) ) {
    ...
    echo json_encode(["status" => "error", "message" => "Email address is missing in request"]);
    exit;
}
...
$sql_user=mysqli_query($conn,"select uid from admin_users where (emailid='".$email."')");
...
try {
    ...
    $mail->isHTML(true);
    $mail->Subject = 'Star Plus Battery : Reset OTP';
    $mail->Body    = "<p>Your login OTP is: <b>$otp</b></p>";
    $mail->AltBody = "Your reset password OTP is: $otp";
