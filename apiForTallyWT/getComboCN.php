...
$doc_no = $jwtf->validateParameter('CRNo.',$docno,STRING);
...
try{
	...
	//if($decode_resp == "SUCCESS_RESPONSE"){
		if($doc_no=="" && $from_date==""){
			$jwtf->returnResponse(FAILED_RESPONSE,$pager,"Date range or Credit Note no. should not be blank");
		}else{
			$resp_cn = $get->getComboCNVoucher($doc_no,$from_date,$to_date);
			if (is_array($resp_cn)){
				$a = array("cnVoucherJson" => $resp_cn);
