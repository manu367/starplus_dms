...
$sql = "SELECT *";
$sql.=" FROM debit_note  where $status $str_filter and create_by = '".$_SESSION['userid']."' and status='Pending For Approval' ";
$query=mysqli_query($link1, $sql) or die("debitnoteapprv-grid-data.php: get  users1");
...
$query=mysqli_query($link1, $sql) or die("debitnoteapprv-grid-data.php: get  users2");
$totalFiltered = mysqli_num_rows($query); // when there is a search parameter then we have to modify total number filtered rows as per search result. 
$sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."  LIMIT ".$requestData['start']." ,".$requestData['length']."   ";
/* $requestData['order'][0]['column'] contains colmun index, $requestData['order'][0]['dir'] contains order such as asc/desc  */
	
$query=mysqli_query($link1, $sql) or die("debitnoteapprv-grid-data.php: get  users3");
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
        ...
        $viewicon = "<div align='center'><a href='view_App_Debit_note.php?ref_no=".$row['ref_no']."&from_date=".$_REQUEST['fromdate']."&to_date=".$_REQUEST['todate']."&location_code=".$_REQUEST['location_code']."&status=".$_REQUEST['status']."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view details'></i></a></div>";
		$datacust = getCustomerDetails($row["cust_id"],"customername",$link1);
		if($datacust == ''){
			 $showdatanew = getVendorDetails($row["cust_id"],"name",$link1);

		}
		else {
			$showdatanew = $datacust;
			}
		$nestedData[] = $j; 
		$nestedData[] = $showdatanew;
		$nestedData[] = getLocationDetails($row["location_id"],"name",$link1);
