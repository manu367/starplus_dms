...
if($username){
	$user_id = " AND b.username ='".$username."'";
}else{
	$user_id = "";
}
//////End filters value/////
...
$sql.=" FROM user_attendence a, admin_users b WHERE 1=1 AND a.user_id=b.username ".$subdeptqry." ".$deptqry."  ".$sqldata." ";
$query=mysqli_query($link1, $sql) or die("attendance-grid-data.php: ERROR-1");
...
$query=mysqli_query($link1, $sql) or die("attendance-grid-data.php: ERROR-2");
$totalFiltered = mysqli_num_rows($query); 
// when there is a search parameter then we have to modify total number filtered rows as per search result. 
$sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."  LIMIT ".$requestData['start']." ,".$requestData['length']." ";
/* $requestData['order'][0]['column'] contains colmun index, $requestData['order'][0]['dir'] contains order such as asc/desc  */	
$query=mysqli_query($link1, $sql) or die("attendance-grid-data.php: ERROR-3");
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
	$nestedData=array(); 
      ...
      if ($row['Image_in'] != '') 
      {
		if($row["insert_date"] < "2022-07-12"){
        $imgin ='<img src="https://eapl1.cansale.in/salesapi/attendanceimg/'.substr($row["insert_date"],0,7).'/'.$row['Image_in'].'"alt="" id="image'.$i.'" onClick="getThisValue('.$i.')" style="width: 100%;"/>';
      }
	  else{
		$imgin ='<img src="../salesapi/attendanceimg/'.substr($row["insert_date"],0,7).'/'.$row['Image_in'].'"alt="" id="image'.$i.'" onClick="getThisValue('.$i.')" style="width: 100%;"/>';
      }}
	//image in//
	...
	//image out//
      ...
      {
		if($row["insert_date"] < "2022-07-12"){
        $imgout ='<img src="https://eapl1.cansale.in/salesapi/attendanceimg/'.substr($row["insert_date"],0,7).'/'.$row['Image_out'].'"alt="" id="image1'.$i.'" onClick="getThisValue('.$i.')" style="width: 100%;"/>';
      }
	  else{
		$imgout ='<img src="../salesapi/attendanceimg/'.substr($row["insert_date"],0,7).'/'.$row['Image_out'].'"alt="" id="image1'.$i.'" onClick="getThisValue('.$i.')" style="width: 100%;"/>';
	  }}
	//image out//
