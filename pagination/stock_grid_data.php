...
if ($_SESSION["userid"] == "admin") {
    $sql = "SELECT id, main_location, from_location,to_location, move_stocktype, doc_no, entry_date, status";
    $sql .= " FROM stock_movement_master WHERE  main_location IN (".$accesslocation.") AND ".$filter_str."";
} else {
    $sql = "SELECT id, main_location, from_location,to_location, move_stocktype, doc_no, entry_date, status";
    $sql .= " FROM stock_movement_master WHERE  main_location IN (".$accesslocation.") AND ".$filter_str." ";
...
// START PRINT //
    $print = "";
    $print = "<a href='../print/print_stockmovement.php?rb=view&id=" . base64_encode($row['doc_no']) . "" . $pagenav . "' class='btn " . $btncolor . "  title='Print Document'><i class='fa fa-print fa-lg' title='Print Document'></i></a>";
