...
// datatable column index  => database column name
	...
	1 => 'site_name',
	2 => 'site_code',
	3 => 'manufacturing_unit',
	4 => 'status'
);

// getting total number records without any search
$sql = "SELECT *";
$sql.=" FROM charging_site_master where ".$status."";
$query=mysqli_query($link1, $sql) or die("chargingsite-grid-data.php: get chargingsite master");
...
if( !empty($gatepassdata['search']['value']) ) {   // if there is a search parameter, $gatepassdata['search']['value'] contains search parameter
	$sql.=" AND (site_name LIKE '".$gatepassdata['search']['value']."%'";
	$sql.=" OR site_code LIKE '".$gatepassdata['search']['value']."%'";
	$sql.=" OR manufacturing_unit LIKE '".$gatepassdata['search']['value']."%'";
...
while( $row=mysqli_fetch_array($query) ) { 
    ...
    $viewicon = "<div align='center'><a href='add_charging_site.php?op=Edit&id=".$row['id']."&status=".$_REQUEST['status']."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view/edit charging site details'></i></a></div>";

	$nestedData[] = $j; 
	$nestedData[] = $row["site_name"];
	$nestedData[] = $row["site_code"];
	$nestedData[] = $row["manufacturing_unit"];
