...
$sql = "SELECT *";
$sql.=" FROM credit_note where $str_filter and $status $location_code";

$query=mysqli_query($link1, $sql) or die("creditnotelist-grid-data.php: get  users1");
...
$query=mysqli_query($link1, $sql) or die("creditnotelist-grid-data.php: get  users2");
$totalFiltered = mysqli_num_rows($query); // when there is a search parameter then we have to modify total number filtered rows as per search result. 
$sql.=" ORDER BY ". $columns[$requestData['order'][0]['column']]."   ".$requestData['order'][0]['dir']."  LIMIT ".$requestData['start']." ,".$requestData['length']."   ";
/* $requestData['order'][0]['column'] contains colmun index, $requestData['order'][0]['dir'] contains order such as asc/desc  */	
$query=mysqli_query($link1, $sql) or die("creditnotelist-grid-data.php: get  users3");
...
while( $row=mysqli_fetch_array($query) ) {  // preparing an array
		...
		##########################################################################################################################3
		 $viewicon = "<div align='center'><a href='view_cr_notes.php?ref_no=".base64_encode($row['ref_no'])."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view details'></i></a></div>";
		
		#############3cancel invoice#############################3
		 if($row['status']!='Cancelled'){ 
                   $cancel = "<div align='center'><a href='cancel_cr_notes.php?ref_no=".base64_encode($row['ref_no'])."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view details'></i></a></div>";
           }

       
		
		###############3  print of invoice ##################
		$print = "<div align='center'><a href='#' onClick=javascript:window.open('../print/print_credit_note.php?ref_no=".base64_encode($row['ref_no'])."','PrintInvoice','toolbar=no,status=no,resizable=yes,scrollbars=yes,width=860,height=700,top=50,left=350')><i class='fa fa-print fa-lg faicon' title='view details'></i></a></div>";

		#############
		$talysync = "";
		if($row['post_in_tally']=="Y"){
			$talysync = '<i class="fa fa-refresh fa-lg text-success" aria-hidden="true" title="sync in tally"></i>';
		}else{
			$talysync = '<i class="fa fa-ban fa-lg  text-danger" aria-hidden="true" title="not sync in tally"></i>'; 
		}
		$nestedData[] = $j; 
		$nestedData[] = $talysync; 
