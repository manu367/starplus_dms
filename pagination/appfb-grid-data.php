...
if($_REQUEST['feedback_by'] !=''){
	$filter_str	.= " AND a.entry_by = '".$_REQUEST['feedback_by']."'";
}
if($_REQUEST['feedback_type'] !=''){
	$filter_str	.= " AND a.problem = '".$_REQUEST['feedback_type']."'";
}
...
$sql = "SELECT a.id";
$sql.=" FROM query_master a, admin_users b WHERE 1=1 AND a.entry_by=b.username ".$subdeptqry." ".$deptqry." ".$filter_str."";
$query=mysqli_query($link1, $sql) or die("fb-grid-data.php: get fb1");
$totalData = mysqli_num_rows($query);
$totalFiltered = $totalData;  // when there is no search parameter then total number rows = total number filtered rows.

$sql = "SELECT a.*,b.name,b.department";
$sql.=" FROM query_master a, admin_users b WHERE 1=1 AND a.entry_by=b.username ".$subdeptqry." ".$deptqry." ".$filter_str."";
if( !empty($requestData['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
	$sql.=" AND (a.query LIKE '".$requestData['search']['value']."%'"; 
	$sql.=" OR a.problem LIKE '".$requestData['search']['value']."%'";
	$sql.=" OR a.module LIKE '".$requestData['search']['value']."%'"; 
	$sql.=" OR a.request LIKE '".$requestData['search']['value']."%'"; 
	$sql.=" OR a.entry_by LIKE '".$requestData['search']['value']."%'";
