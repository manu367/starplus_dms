...
if($_REQUEST['prod_cat']!=""){
	$prodcat="productid='".$_REQUEST['prod_cat']."'";
}else{
	$prodcat="1";
}
$columns = array( 
...
$sql = "SELECT *";
$sql.=" FROM product_sub_category where ".$status." and ".$prodcat;
////$sql.=" FROM price_master where ".$loc_state." and ".$product." and ".$loc_type."";
$query=mysqli_query($link1, $sql) or die("prodsubcat-grid-data.php: get prodsubcat master");
...
if( !empty($gatepassdata['search']['value']) ) {   // if there is a search parameter, $gatepassdata['search']['value'] contains search parameter
	$sql.=" AND ( prod_sub_cat LIKE '".$gatepassdata['search']['value']."%'";    
	$sql.=" OR product_category LIKE '".$gatepassdata['search']['value']."%'";    
...
if($row['status'] == '1'){ $status = "Active";}
 ...
 // preparing an array
	...
	////// check this user have right to view the details
        ...
        $viewicon = "<div align='center'><a href='add_prod_subcat.php?op=Edit&id=".$row['psubcatid']."&status=".$_REQUEST['status']."&prod_cat=".$_REQUEST['prod_cat']."".$pagenav."' title='view'><i class='fa fa-eye fa-lg faicon' title='view/edit product sub category details'></i></a></div>";
	
   
     
	$nestedData[] = "<div align='center'>".$j."</div>"; 
	$nestedData[] = $row["prod_sub_cat"];
