...
try{
	...
	if($decode_resp == "SUCCESS_RESPONSE"){
		...
		if($rowu["status"]=="active"){
			////// track user activity
			$resp = $pst->updateUserActivity($userCode,"User Travel Plan","Update-".$inStatus."".$outStatus,$lati,$longi,$userId,$trackaddrs,$trackdistc);
			if($resp){
				$upd_attend = explode("~",$pst->updateUserTravel($userId,$userCode,$inLatitude,$inLongitude,$inDatetime,$inStatus,$inAddress,$inImage,$inImageName,$outLatitude,$outLongitude,$outDatetime,$outStatus,$outAddress,$outImage,$outImageName));
				if($upd_attend[0] == "1"){
					//////////
