...
if($_POST){
 if ($_POST['upd']=='Save'){
	...
	if(substr($partycode,0,4)=="EADL"){
		...
		$flag = approvalActivity($po_no,$today,"PO",$_REQUEST['usercode'],$actiontaken,$today,$currtime,"AUTO APPROVED",$ip,$link1,$flag);
		////// insert in activity table////
		$flag = dailyActivity($_REQUEST['usercode'],$po_no,"PO APPROVAL","APPROVAL",$ip,$link1,$flag);
	}
	...
	header("Location:processpage.php?respmsg=".base64_encode($msg)."&usercode=".$_REQUEST['usercode']."&latitude=".$_REQUEST['latitude']."&longitude=".$_REQUEST['longitude']."&trackaddress=".$_REQUEST['trackaddress']."&taskid=".$_REQUEST['taskid']."&respheadmsg=".$respheadmsg."&headerline=".$headerline);
    exit;
 }
}
?>
...
function get_price(ind){
	...
	var pricestate=price_pickstr.split("~");
		...
		success:function(data){
			...
			if(splitprice[1]){ var mrpp = splitprice[1];}else{ var mrpp = 0.00;}
			if(splitprice[0]){ var pricc = splitprice[0];}else{ var pricc = 0.00;}
	        document.getElementById("price["+ind+"]").value=formatCurrency(pricc);
			document.getElementById("holdRate["+ind+"]").value=formatCurrency(pricc);
		    document.getElementById("mrp["+ind+"]").value=formatCurrency(mrpp);
	    }
	  });
...
<div class="container-fluid">
  <div class="row content">
    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
      ...
      <div align="center"><button title="Refresh" type="button" class="btn<?=$btncolor?>" onClick="window.location.href='postNewPO.php?usercode=<?=$_REQUEST["usercode"]?>&latitude=<?=$_REQUEST['latitude']?>&longitude=<?=$_REQUEST['longitude']?>'"><i class="fa fa-refresh fa-lg" title="Refresh"></i>&nbsp;&nbsp;<span>Refresh</span></button></div>
      <?php //if($_REQUEST["usercode"]=="admin" || $_REQUEST["usercode"]=="EAUSR677"){ ?>
      <?php /*?><div style="display:inline-block; float:right"><button title="Add Combo SO" type="button" class="btn<?=$btncolor?>" onClick="window.location.href='postNewCPO.php?usercode=<?=$_REQUEST["usercode"]?>&latitude=<?=$_REQUEST['latitude']?>&longitude=<?=$_REQUEST['longitude']?>'"><i class="fa fa-shopping-basket fa-lg" title="Add Combo SO"></i>&nbsp;&nbsp;<span>Add Combo SO</span></button></div><?php */?>
      <?php //}?>
      <br/>
        ...
        <form id="frm1" name="frm1" class="form-horizontal" action="" method="post">
          ...
          <div class="form-group">
            <div class="col-md-10">
              ...
              <div class="col-md-7">
                <select name="po_to" id="po_to" required class="form-control required selectpicker" data-live-search="true" onChange="document.frm1.submit();">
                  <option value="" selected="selected">Please Select </option>
                  <?php 
