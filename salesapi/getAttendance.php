...
try{
	...
	if($decode_resp == "SUCCESS_RESPONSE"){
		$res_attend = $get->getAttendance($user_code,$from_date,$to_date);
		if ($res_attend != false){
			$row = mysqli_fetch_array($res_attend);
			$a = array("userid" => $user_id, "usercode" => $user_code, "intime" => $row["in_datetime"], "outtime" => $row["out_datetime"], "statusin" => $row["status_in"], "statusout" => $row["status_out"], "inlatitude" => $row["latitude_in"], "inlongitude" => $row["longitude_in"], "outlatitude" => $row["latitude_out"], "outlongitude" => $row["longitude_out"], "inaddress" => $row["address_in"], "outaddress" => $row["address_out"], "inimage" => ATTACHMENT_URL."attendanceimg/".substr($row["insert_date"],0,7)."/".$row["Image_in"], "outimage" => ATTACHMENT_URL."attendanceimg/".substr($row["insert_date"],0,7)."/".$row["Image_out"]);
