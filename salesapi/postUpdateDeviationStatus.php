...
try{
	...
	if($decode_resp == "SUCCESS_RESPONSE"){
		////// track user activity
		$resp = $pst->updateUserActivity($user_code,"Devaiation Approval","Update",$lati,$longi,$user_id,$trackaddrs,$trackdistc);
		if($resp){
			$upd_act = explode("~",$pst->updateDeviationStatus($user_id,$user_code,$act_id,$system_refno,$remark,$status,$doc_encodestr,$doc_name,$lati,$longi,$trackaddrs));
			if($upd_act[0] == "1"){
