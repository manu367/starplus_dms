...
if($_POST){
 if ($_POST['upd']=='Save'){
	...
	///// Insert Master Data
	$ref_no = $_REQUEST["userid"];
	///// Insert Master Data
	$query1= "INSERT INTO purchase_order_master set po_to='".$parentcode."',po_from='".$partycode."',po_no='".$po_no."',temp_no='".$c_nos."',ref_no='".$ref_no."',requested_date='".$today."',entry_date='".$today."',entry_time='".$currtime."',req_type='COMBO PO',status='PFA',po_value='".$sub_total."',create_by='".$_REQUEST['usercode']."',ip='".$ip."',sales_person='".$se."',sales_executive='".$sales_executive1."',payment_status='".$payment_terms."',transport_exp='',discount='".$total_discount."',discount_type='PD',remark='".$remark."',delivery_address='".$delivery_address."',address='".$trackaddrs."',latitude='".$lat."',longitude='".$long."',pjp_id='".$_REQUEST['task_id']."',entry_from='APP', sale_type='".$saletype."'";
	...
	{   
		...
		if($prod_code[$k]!='' && $req_qty[$k]!='' && $req_qty[$k]!=0) {
			...
			/////////// insert data
		   $query2="insert into purchase_order_data set po_no='".$po_no."', prod_code='".$val."',prod_cat='C',psc_id='".$proddet[1]."',brand_id='', req_qty='".$req_qty[$k]."', po_price='".$price[$k]."', po_value='".$linetotal[$k]."', hold_price='".$holdRate[$k]."', mrp='".$mrp[$k]."', discount='".$rowdiscount[$k]."', totalval='".$total_val[$k]."',warranty='COMBO',expected_deliv_date='".$deliv_date[$k]."', sale_type='".$saletype."'";
	...
	if($_REQUEST['taskid']){
   	...
   	$result = mysqli_query($link1,"INSERT INTO user_track SET userid='".$_REQUEST['usercode']."', task_name='Combo Sales Order', task_action='Create', ref_no='".$po_no."', latitude='".$_REQUEST['latitude']."', longitude='".$_REQUEST['longitude']."', address='".$trackaddrs."',travel_km='".$trackdistc."', remote_address='".$_SERVER['REMOTE_ADDR']."',remote_agent='".$_SERVER['HTTP_USER_AGENT']."' , entry_date='".$today."'");
	//// check if query is not executed
	if (!$result) {
		 $flag = false;
	...
	$flag=dailyActivity($_REQUEST['usercode'],$po_no,"CPO","ADD",$ip,$link1,$flag);
	...
	if(substr($partycode,0,4)=="EADL"){
		...
		$flag = approvalActivity($po_no,$today,"CPO",$_REQUEST['usercode'],$actiontaken,$today,$currtime,"AUTO APPROVED",$ip,$link1,$flag);
		////// insert in activity table////
		$flag = dailyActivity($_REQUEST['usercode'],$po_no,"CPO APPROVAL","APPROVAL",$ip,$link1,$flag);
	}
    ...
    mysqli_close($link1);
	$headerline = "Combo Sales Order";
	///// move to parent page
    //header("location:purchaseOrderList.php?msg=".$msg."".$pagenav);
	header("Location:processpage.php?respmsg=".base64_encode($msg)."&usercode=".$_REQUEST['usercode']."&latitude=".$_REQUEST['latitude']."&longitude=".$_REQUEST['longitude']."&taskid=".$_REQUEST['taskid']."&trackaddress=".$_REQUEST['trackaddress']."&respheadmsg=".$respheadmsg."&headerline=".$headerline);
    exit;
 }
}
?>
...
function getSpecification(comboid,indx){
	$.ajax({
		...
		success:function(data){
			...
			if(getdata[0]){
				document.getElementById("prd_desc"+getdata[1]+"").innerHTML = getdata[0];
			}else{
				document.getElementById("prd_desc"+indx+"").innerHTML = "";
			}
			//rePop();
...
$(document).ready(function(){
     $("#add_row").click(function(){
		...
		numi.value = num;
		r = '<div class="form-group" id="addr'+num+'" style="padding-top: 8px; padding-bottom: 8px; border:ridge; background-color: aliceblue; margin-right: 0px"><div class="col-md-6"><label class="col-md-6 control-label">Combo <span class="red_small">*</span></label><div class="col-md-6"><select name="prod_code['+num+']" id="prod_code['+num+']" required onchange="getAvlStk('+num+');checkDuplicate(' + num + ',this.value);get_price('+num+');" class="form-control required dynmprod"><option value="">--Please Select--</option></select><span id="prd_desc'+num+'"></span></div></div><div class="col-md-6"><label class="col-md-5 control-label">Qty <span class="red_small">*</span></label><div class="col-md-6"><input type="text" name="req_qty['+num+']" id="req_qty['+num+']" onblur=rowTotal('+num+');myFunction(this.value,'+num+',"req_qty"); class="digits form-control" onkeypress="return onlyNumbers(this.value);" maxlength="3"/></div></div><div class="col-md-6"><label class="col-md-6 control-label">Price <span class="red_small">*</span></label><div class="col-md-6"><input  name="price['+num+']" id="price['+num+']" type="text" onkeypress="return onlyFloatNum(this.value)" class="required form-control" onblur="rowTotal('+num+');"></div></div><div class="col-md-6"><label class="col-md-5 control-label">Value</label><div class="col-md-6"><input type="text" class="form-control" name="linetotal['+num+']" id="linetotal['+num+']" autocomplete="off" readonly><input type="hidden" class="form-control" name="rowdiscount['+num+']" id="rowdiscount['+num+']" onkeypress="return onlyFloatNum(this.value);" autocomplete="off" onblur="rowTotal('+num+');"></div></div><div class="col-md-6"><label class="col-md-6 control-label">Total</label><div class="col-md-6"><input type="text" class="form-control" name="total_val['+num+']" id="total_val['+num+']" autocomplete="off" readonly><input name="mrp['+num+']" id="mrp['+num+']" type="hidden"/><input name="holdRate['+num+']" id="holdRate['+num+']" type="hidden"/></div></div><input name="warranty_days['+num+']" id="warranty_days['+num+']" type="hidden" class="form-control" readonly/><input name="deliv_date['+num+']" id="deliv_date['+num+']" type="hidden" class="form-control" readonly/><div class="col-md-6"><label class="col-md-6 control-label">&nbsp;</label><div class="col-md-6"><i class="fa fa-close fa-lg" onClick="deleteRow('+num+');"></i></div></div></div>';
		$('#so_items').append(r);
		makeSelect();
		}
  });
...
<div class="container-fluid">
  <div class="row content">
    <div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
      <h2 align="center"><i class="fa fa-shopping-basket"></i> Add New SO For COMBO </h2>
      <div align="center"><button title="Refresh" type="button" class="btn<?=$btncolor?>" onClick="window.location.href='postNewCPO.php?usercode=<?=$_REQUEST["usercode"]?>&latitude=<?=$_REQUEST['latitude']?>&longitude=<?=$_REQUEST['longitude']?>'"><i class="fa fa-refresh fa-lg" title="Refresh"></i>&nbsp;&nbsp;<span>Refresh</span></button></div>
<?php /*?>      <div style="display:inline-block; float:right"><button title="Add SO" type="button" class="btn<?=$btncolor?>" onClick="window.location.href='postNewPO.php?usercode=<?=$_REQUEST["usercode"]?>&latitude=<?=$_REQUEST['latitude']?>&longitude=<?=$_REQUEST['longitude']?>'"><i class="fa fa-shopping-basket fa-lg" title="Add SO"></i>&nbsp;&nbsp;<span>Add SO</span></button></div><?php */?>
      <br/>
      <div class="form-group" id="page-wrap" style="margin-left:10px;">
        <form id="frm1" name="frm1" class="form-horizontal" action="" method="post">
          <div class="form-group">
            <div class="col-md-10">
              ...
              <div class="col-md-7">
                <?php /*?><select name="po_from" id="po_from" required class="form-control selectpicker required" data-live-search="true" onChange="document.frm1.submit();">
          ...
          <div class="form-group">
            <div class="col-md-10">
              ...
              <div class="col-md-7">
                <select name="po_to" id="po_to" required class="form-control required" data-live-search="true" onChange="document.frm1.submit();">
                  <option value="" selected="selected">Please Select </option>
                  <?php 
					...
					if(mysqli_num_rows($res_parent)>0){
						while($result_parent=mysqli_fetch_array($res_parent)){
							  ...
							  ?>
					  <option value="<?=$result_parent['uid']?>" <?php if($result_parent['uid']==$_REQUEST['po_to'])echo "selected";?> >
              ...
              <div class="col-md-7">
                <select name="sales_executive" id="sales_executive" required class="form-control selectpicker required" data-live-search="true" onChange="document.frm1.submit();">
                  ...
                  <?php 
                    ...
                    ?>
                  <option value="<?=$result_se['username']?>" <?php if($result_se['username']==$_REQUEST['usercode']) { echo "selected"; } ?> ><?=$result_se['name']." | ".$result_se['username']?></option>
                  <?php
					}
                    ?>
                </select>
        ...
        <div class="text-danger" align="center"><?php if($toloctiondet[1]=="DS"){ echo "You are creating PRIMARY order";}else if($toloctiondet[1]=="DL"){echo "You are creating SECONDARY order";}else{}?></div>
        <form id="frm2" name="frm2" class="form-horizontal" action="" method="post">
          <div class="form-group" id='addr0' style="padding-top: 8px; padding-bottom: 8px; border:ridge; background-color: aliceblue; margin-right: 0px">
            <div class="col-md-6"><label class="col-md-6 control-label">Combo <span class="red_small">*</span></label>
                <div class="col-md-6">
                      <select name="prod_code[0]" id="prod_code[0]" class="form-control required dynmprod" onChange="getAvlStk(0);get_price(0);checkDuplicate(0, this.value);">
                          <option value=''>--Please Select--</option>
                      </select>
            ...
            <div class="col-md-6"><label class="col-md-5 control-label">Qty <span class="red_small">*</span></label>
                <div class="col-md-6">
                    <input type="text" class="form-control digits" name="req_qty[0]" id="req_qty[0]"  autocomplete="off" required onBlur="myFunction(this.value,0,'req_qty');rowTotal(0);" onKeyPress="return onlyNumbers(this.value);" maxlength="3">
                </div>
            </div>
        ...
        <div class="row">
          ...
          <div class="form-group">
            <div class="col-md-10">
              ...
              <div class="col-md-3">
                <textarea name="delivery_address" id="delivery_address" class="form-control required" style="resize:none" required><?=$toloctiondet[2]?>
</textarea>
              </div>
