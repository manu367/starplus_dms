...
try{
	...
	if($decode_resp == "SUCCESS_RESPONSE"){
		$res_collectionlist = $get->getCollectionList($user_code,$from_date,$to_date,$task_id);
		if ($res_collectionlist != false){
			$a = array();
			$b = array();
			//while($row = mysqli_fetch_array($res_collectionlist)){
				
				$b["refId"] = "1";
				$b["status"] = "In Process";
				$b["updatedate"] = date("Y-m-d H:i:s");
				array_push($a,$b);
			//}
			$c = array("userid" => $user_id, "usercode" => $user_code, "collectionlist" => $a);
