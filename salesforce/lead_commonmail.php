<?php
$lead = mysqli_query($link1,"select  * from sf_lead_master where lid='".$id."'");
$lrow = mysqli_fetch_assoc($lead);

//$maildata =  mysqli_fetch_array(mysqli_query($link1,"select email ,empname from hrms_employe_master where empid = '".$lrow['dept']."' "));

$sender_data = mysqli_fetch_array(mysqli_query($link1,"select emailid , name from admin_users where username = '".$_SESSION['userid']."' "));



$to_email = "$lrow[party_email]";
$cc_mail = "sulakshna@candoursoft.com";
$subject_email = "Lead Note";
$msg_email = "<html>
			<head>
			<title>Lead Note</title>
			</head>
			<body>
			<p>Dear ".$lrow['partyid'].",</p>
			<p>This is to inform  that your lead no. is  ".str_replace(",","  ,  ",$lrow['reference'])." updated with remark <b><i>".$inote." ".$cnote."</b></i> </p>";
...
function send_mail_fun($content,$send_to,$send_cc){
	...
	$mail->Username = "shekhar@candoursoft.com";

	//Password to use for SMTP authentication
	$mail->Password = "css@321A";

	//Set who the message is to be sent from
	$mail->setFrom('$sender_data[emailid]', 'Lead Note');
