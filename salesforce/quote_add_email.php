...
$msg_email = "<html>
			...
			<body>
			<p>Dear ".$party_id.",</p>
			<p>This is to inform  that your quote no. ".str_replace(",","  ,  ",$auto_quote_no)." is created successfully</p>";
...
function send_mail_fun($content,$send_to,$send_cc){
	...
	$mail->setFrom('$sender_data[emailid]', 'Quote Data');
	...
	if (!$mail->send()) {
		$err_msg =  "Mailer Error: " . $mail->ErrorInfo;
		  ...
		  //$msgg="Quote added successfully with reference ".$auto_quote_no."";
		  //header("Location:quote_list.php?msg=$msgg&sts=success&page=lead".$pagenav);
	}
}
?>