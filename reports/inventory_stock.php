...
$array_loc = array();
if(!empty($_REQUEST['locationcode'])){ $arr_selstatus = $_REQUEST['locationcode'];}else{ $arr_selstatus = array();}
for($i=0; $i<count($arr_selstatus); $i++){
	$array_loc[$arr_selstatus[$i]] = "Y";
}
@extract($_POST);
## selected location
/*if($locationcode!=""){
...
if(is_array($_REQUEST['locationcode'])){
	$statusstr="";
	$post_statusarr = $_REQUEST['locationcode'];
...
<div class="container-fluid">
  <div class="row content">
    ...
    <div class="col-sm-9 tab-pane fade in active" id="home">
      <h2 align="center"><i class="fa fa-cubes fa-lg"></i>Stock</h2><br/>
      <div class="form-group" id="page-wrap" style="margin-left:10px;">
      <form class="form-horizontal" role="form" name="frm1" id="frm1" action="" method="post">
      <div class="row">
        <div class="col-sm-3" align="right"><label class="control-label">State</label></div>
        <div class="col-sm-3">
         ...
         <div class="col-sm-3" align="right"><label class="control-label">Inventory Type</label></div>
         <div class="col-sm-3"><select name="inventorytype" id="inventorytype" class="form-control" required>
                    <option value="" selected="selected">Please Select </option>
                    <option value="imei"<?php if($_REQUEST['inventorytype']=="imei"){ echo "selected";}?>>Serial No. INVENTORY</option>
      ...
      <div class="row">
        ...
        <div class="col-sm-9"><select name="locationcode[]" id="locationcode" class="form-control" multiple="multiple">
               
                    <?php 
					if($_REQUEST['locationstate']){
						$sql_chl="select * from access_location where uid='".$_SESSION['userid']."' and status='Y' AND state='".$_REQUEST['locationstate']."'";
					}else{
					...
					while($result_chl=mysqli_fetch_array($res_chl)){
                          ...
                          ?>
                    <option data-tokens="<?=$party_det['name']." | ".$result_chl['location_id']?>" value="<?=$result_chl['location_id']?>" <?php if($array_loc[$result_chl['location_id']]=="Y"){ echo "selected";}?> >
      ...
      <div class="row">
        ...
        <div style="float:right">
            <?php
			   ...
			   ?>
        <strong>Excel Export</strong>&nbsp;&nbsp;&nbsp;&nbsp; <a href="excelexport.php?rname=<?=base64_encode("inventory")?>&rheader=<?=base64_encode("Inventory")?>&loc=<?=base64_encode(json_encode($_POST['locationcode']))?>&product_cat=<?=base64_encode($_POST['product_cat'])?>&product_subcat=<?=base64_encode($_POST['product_subcat'])?>&brand=<?=base64_encode($_POST['brand'])?>&partcode=<?=base64_encode($_POST['partcode'])?>" title="Export inventory details in excel" style="float:right"><i class="fa fa-file-excel-o fa-2x" title="Export inventory details in excel"></i></a><br/><br/>
        <?php
				//}
				?>
                </div>
       <table  width="99%" id="myTable" class="table-striped table-bordered table-hover" align="center">
          ...
          <tbody>
			...
			$new_loc = "";
			$old_loc = "";
			$sql=mysqli_query($link1,"Select a.asc_code, a.sub_location, a.partcode, a.okqty, a.broken, a.missing, b.productname, b.productcolor FROM stock_status a, product_master b WHERE a.partcode=b.productcode AND ".$loc." AND ".$pcat_s." AND ".$pscat_s." AND ".$brd_s." AND ".$part_code);
      ...
      <?php }?>
      <?php if($_REQUEST['inventorytype']=="imei"){ ?>
      <div class="row">
        <div class="col-sm-12 table-responsive">
        <div style="float:right">
            <?php
			    ...
			    //if(getExcelRight($_SESSION['userid'],$processid,$link1)==1){
			   ?>
        <strong>Excel Export</strong>&nbsp;&nbsp;&nbsp;&nbsp; <a href="excelexport.php?rname=<?=base64_encode("serialstock")?>&rheader=<?=base64_encode("Serial Inventory")?>&loc=<?=base64_encode(json_encode($_POST['locationcode']))?>&product_cat=<?=base64_encode($_POST['product_cat'])?>&product_subcat=<?=base64_encode($_POST['product_subcat'])?>&brand=<?=base64_encode($_POST['brand'])?>&partcode=<?=base64_encode($_POST['partcode'])?>" title="Export inventory details in excel" style="float:right"><i class="fa fa-file-excel-o fa-2x" title="Export inventory details in excel"></i></a><br/><br/>
        <?php
				//}
				?>
                </div>
       <?php /*?><table  width="99%" id="myTable" class="table-striped table-bordered table-hover" align="center">
          <thead>
            <tr class="<?=$tableheadcolor?>" >
              <th><a href="#" name="entity_id" title="asc" ></a>S.No</th>
              <th data-class="expand"><a href="#" name="name" title="asc" ></a>Location Name</th>
              ...
              <th data-hide="phone"><a href="#" name="name" title="asc" class="not-sort"></a>Product Code</th>
              <th data-hide="phone,tablet"><a href="#" name="name" title="asc" class="not-sort"></a>Product Name</th>
              <th data-hide="phone,tablet"><a href="#" name="number" title="asc" class="not-sort"></a>Serial No.1</th>
              <th data-hide="phone,tablet"><a href="#" name="number" title="asc" class="not-sort"></a>Stock Type</th>
              <th data-hide="phone,tablet"><a href="#" name="number" title="asc" class="not-sort"></a>Aging</th>
              </tr>
          </thead>
          <tbody>
            <?php
			$sno=0;
	//echo "Select a.owner_code, a.prod_code, a.imei1, a.stock_type, b.productname, b.productcolor, b.productcategory  from billing_imei_data a, product_master b WHERE a.prod_code=b.productcode AND ".$imeiloc." AND ".$pcat_s." AND ".$pscat_s." AND ".$brd_s." AND ".$part_code;
			//echo "Select a.owner_code, a.prod_code, a.imei1, a.stock_type, b.productname, b.productcolor, b.productcategory  from billing_imei_data a, product_master b WHERE a.prod_code=b.productcode AND ".$imeiloc." AND ".$pcat_s." AND ".$pscat_s." AND ".$brd_s." AND ".$part_code;
			$sql=mysqli_query($link1,"Select a.owner_code, a.prod_code, a.imei1, a.stock_type, b.productname, b.productcolor, b.productcategory  from billing_imei_data a, product_master b WHERE a.prod_code=b.productcode AND ".$imeiloc." AND ".$pcat_s." AND ".$pscat_s." AND ".$brd_s." AND ".$part_code)or die(mysqli_error($link1));
			while($row=mysqli_fetch_assoc($sql)){
				$checkstatus ="";
				...
				if($chek_rcvin['status']==""){
				  ...
				  ////get product cat
				  $prod_cat = explode("~",getAnyDetails($row['productcategory'],"cat_name,short_code","catid","product_cat_master",$link1));
				  ////// if serial no. search for all charged battery
				  if($prod_cat[1]=="BTR"){
				  		$serialinfo = checkSerialNoLogic(strtoupper($row['imei1']),"BTR");
				  ...
				  ////// if serial no. search for lithium ion battery
				  		...
				  		$serialinfo = checkSerialNoLogic(strtoupper($row['imei1']),"LTHIBTR");
						///// get manufacturing date
						$bt_mfd = getMfYear($serialinfo["mf_year"])."-".date("m", strtotime(getMfMonth($serialinfo["mf_month"])))."-".str_pad(getMfDate($serialinfo["mf_date"]),2,0,STR_PAD_LEFT);
						$mfd = $bt_mfd;
				  }
				  ////// if serial no. search for E-Rickshaw battery charger
						...
						$serialinfo = checkSerialNoLogic(strtoupper($row['imei1']),"ERBTRCHR");
						...
						$serialinfo = checkSerialNoLogic(strtoupper($row['imei1']),"SOL");
						///// get manufacturing month
						$sol_mfm = date("m", strtotime(getMfMonth($serialinfo["mf_month"])));
						///// get manufacturing year
						$sol_mfy = getMfYear($serialinfo["mf_year"]);
						$mfd = $sol_mfy."-".$sol_mfm."-01";
				  }
				  //else if($prod_cat[1]=="POE" || $prod_cat[1]=="UPS" || $prod_cat[1]=="EXP"){
				  else{
				  		$serialinfo = checkSerialNoLogic(strtoupper($row['imei1']),"SOLELC");
						///// get manufacturing month
						$sol_mfm = date("m", strtotime(getMfMonth($serialinfo["mf_month"])));
						///// get manufacturing year
            ...
            <tr class="even pointer">
              <td><?php echo $sno;?></td>
              <td><?php echo $locdet[0].",".$locdet[1].",".$row['owner_code'];?></td>
              <td><?=getLocationType($locdet[3],$link1);?></td>
              <td><?php echo $row['prod_code'];?></td>
              <td><?php echo $row['productname'].",".$row['productcolor'];?></td>
              <td align="left"><?php echo $row['imei1'];?></td>
              <td align="left"><?php echo $row['stock_type'];?></td>
              <td align="left"><?php echo daysDifference($today,$mfd);?></td>
              </tr>
            <?php } }?>
          </tbody>
          </table><?php */?> 
