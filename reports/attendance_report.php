              ...
              <form class="form-horizontal" role="form" name="form1" action="" method="post">
              	<div class="row">
                    ...
                    <div class="col-sm-3 col-md-3 col-lg-3"><br/>
                        <a href="excelexport.php?rname=<?=base64_encode("attendanceReport")?>&rheader=<?=base64_encode("Attendance")?>&user_id=<?=base64_encode($_REQUEST['username'])?>&fromDate=<?=base64_encode($_REQUEST['fdate'])?>&toDate=<?=base64_encode($_REQUEST['tdate'])?>&department=<?=base64_encode($_REQUEST['department'])?>&subdepartment=<?=base64_encode($_REQUEST['subdepartment'])?>" title="Export user details in excel"><i class="fa fa-file-excel-o fa-2x" title="Export user details in excel"></i></a>
                        
                    </div>
                    <div class="col-sm-3 col-md-3 col-lg-3"><br/>
                    	<a href="excelexport.php?rname=<?=base64_encode("summarizedAttendRep")?>&rheader=<?=base64_encode("Attendance")?>&user_id=<?=base64_encode($_REQUEST['username'])?>&fromDate=<?=base64_encode($_REQUEST['fdate'])?>&toDate=<?=base64_encode($_REQUEST['tdate'])?>&department=<?=base64_encode($_REQUEST['department'])?>&subdepartment=<?=base64_encode($_REQUEST['subdepartment'])?>" title="Export attendance with Absent/Present in excel"><i class="fa fa-file-excel-o fa-2x" title="Export attendance with Absent/Present in excel"></i></a>
                    </div>
                  </div>
              </form>
              <br/>
                ...
                <table  width="99%" id="myTable" class="table-striped table-bordered table-hover" align="center">
                  ...
                  <tbody>
					...
					if($_REQUEST['Submit']!=""){
                            ...
                            $tod = $_REQUEST['tdate'];
                            $sqldata = "SELECT a.*, b.name, b.oth_empid, b.department, b.subdepartment FROM user_attendence a, admin_users b WHERE 1=1 AND a.user_id=b.username ".$subdeptqry." ".$deptqry." ".$user_id;
                    ...
                    <tr class="even pointer">
                      ...
                      <td align='center' width='8%'><?php if ($row['Image_in'] != '') { ?>
						  <?php if($row["insert_date"] < "2022-07-12"){?>
						  <img src="https://eapl1.cansale.in/salesapi/attendanceimg/<?=substr($row["insert_date"],0,7)?>/<?=$row['Image_in'];?>" alt="" id="image<?=$i?>" onClick="getThisValue(<?=$i?>)" style="width: 100%;"/>
						  <?php }else{?>
                          <img src="../salesapi/attendanceimg/<?=substr($row["insert_date"],0,7)?>/<?=$row['Image_in'];?>" alt="" id="image<?=$i?>" onClick="getThisValue(<?=$i?>)" style="width: 100%;"/>
                        <?php } } ?></td>
                      <td><?= ($row['out_datetime']); ?></td>
                      <td><?= $newstring = wordwrap($row['address_out'], 30, "<br>", 1); ?></td>
                      <td align='center' width='8%'><?php if ($row['Image_out'] != '') { ?>
						  <?php if($row["insert_date"] < "2022-07-12"){?>
						  <img src="https://eapl1.cansale.in/salesapi/attendanceimg/<?=substr($row["insert_date"],0,7)?>/<?= $row['Image_out']; ?>" alt="" id="image1<?=$i?>" onClick="getThisValue(<?='1'.$i?>)" style="width: 100%;"/>
						  <?php }else{?>
                          <img src="../salesapi/attendanceimg/<?=substr($row["insert_date"],0,7)?>/<?= $row['Image_out']; ?>" alt="" id="image1<?=$i?>" onClick="getThisValue(<?='1'.$i?>)" style="width: 100%;"/>
                        <?php }} ?></td>
                    </tr>
                    <?php $i++; }} ?>
                  </tbody>
