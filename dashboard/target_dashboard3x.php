                    ...
                    <tbody>
                        ...
                        if($_REQUEST['selmonth']!="" && $_REQUEST['selyear']!="" && $_REQUEST['user_id']!=""){
							...
							function getDlAchive($link1, $id, $task, $m, $y ,$subcat){
								...
								$resp = 0;
								$dv = ($task == 'Dealer Visit')?true:false;
								...
								if($invcnt_res){
									if(mysqli_num_rows($invcnt_res) > 0){
										while($row = mysqli_fetch_assoc($invcnt_res)){
											...
											$display = '';
											$dv = ($row['task_name'] == 'Dealer Visit')?true:false;
											...
											if($dv){
												...
												$display_a = "Old: ".$ac_arr_old."<br>New: ".$ac_arr_new;
												...
												$display_b = "Old: ".($ac_arr_old + $dlac_arr_old)."<br>New: ".($ac_arr_new + $dlac_arr_new);
											}
											...
											echo '<tr><td>'.$userinfo.'<br>('.$id.')</td><td>'.$row['prod_code'].'</td><td>'.$row['task_name'].'</td><td>'.$row['remark'].'</td><td>'.$row['target_val'].'</td><td>'.$display_a.'</td><td>'.$display_b.'</td><td><div class="progress" style="margin-bottom:0px;"><div class="progress-bar progress-bar-'.$cls.' progress-bar-striped" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width:'.$per.'%" title="'.$per.'% Achieve">'.$per.'% '.$txt.'</div></div></td></tr>';
										}
									}
									else{
										//echo "<br>NO task!<br>";
