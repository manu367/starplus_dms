...
<div class="container-fluid">
  <div class="row content">
    ...
    <div class="col-sm-9 tab-pane fade in active" id="home">
     <h2 align="center"><i class="fa fa-bullseye"></i> Sales Force Dashboard</h2>
...
if($_REQUEST['user_id']){
$i=1;
$res_mng = mysqli_query($link1,"SELECT username, name, oth_empid, designationid FROM admin_users WHERE username='".$_REQUEST['user_id']."'");
...
<div id="accordion">
	<div class="card">
        ...
        <table id="myTable2" class="dt_table" width="100%" border="0" cellpadding="2"  cellspacing="0">
			...
			$low_perf = array();
			$non_perf = array();
			$sql_tar = "SELECT SUM(target_val) AS target_val, task_name, prod_code FROM sf_target_data WHERE status='Active' ".$filter_str." AND user_id IN (".$team.") group by task_name,prod_code,month,year";
			$res_tar = mysqli_query($link1, $sql_tar);
			while($row_tar = mysqli_fetch_assoc($res_tar)){
				////achive
				if($row_tar['task_name'] == "Dealer Visit"){
					//// calculate from old dealer vist
					$row_oldcnt =  mysqli_fetch_assoc(mysqli_query($link1,"SELECT COUNT(id) AS olddealer FROM dealer_visit WHERE userid IN (".$team.") AND visit_date >= '".$_REQUEST['fdate']."' AND visit_date <= '".$_REQUEST['tdate']."' AND dealer_type='Old'"));
					//// calculate from new dealer vist
					$row_newcnt =  mysqli_fetch_assoc(mysqli_query($link1,"SELECT COUNT(id) AS newdealer FROM dealer_visit WHERE userid IN (".$team.") AND visit_date >= '".$_REQUEST['fdate']."' AND visit_date <= '".$_REQUEST['tdate']."' AND dealer_type='New'"));
					//$resp = $row_oldcnt["olddealer"]."~".$row_newcnt["newdealer"];
					$resp = $row_oldcnt["olddealer"]+$row_newcnt["newdealer"];
				}
				else if($row_tar['task_name'] == "Feedback"){
					/// get feedback count
					$row_fb =  mysqli_fetch_assoc(mysqli_query($link1,"SELECT COUNT(id) AS feedback FROM query_master WHERE entry_by IN (".$team.") AND entry_date >= '".$_REQUEST['fdate']."' AND entry_date <= '".$_REQUEST['tdate']."'"));
					//if($row_fb["feedback"]){ echo $ach = $row_fb["feedback"];}else{echo $ach = 0;}
					$resp = $row_fb["feedback"];
				}
				else if($row_tar['task_name'] == "Sale Order"){
					/// get sale order count (pri)
					$row_so_pri =  mysqli_fetch_assoc(mysqli_query($link1,"SELECT SUM(req_qty) AS socnt FROM purchase_order_data WHERE po_no IN (SELECT po_no FROM purchase_order_master WHERE sale_type LIKE 'PRIMARY' AND create_by IN (".$team.") AND entry_date >= '".$_REQUEST['fdate']."' AND entry_date <= '".$_REQUEST['tdate']."' AND status IN ('Approved','Processed')) AND prod_code IN (SELECT productcode FROM product_master WHERE productsubcat IN (SELECT psubcatid FROM product_sub_category WHERE prod_sub_cat='".$row_tar['prod_code']."'))"));
				...
				else if($row_tar['task_name'] == "Collection"){
					/// get collection count
					$row_col =  mysqli_fetch_assoc(mysqli_query($link1,"SELECT SUM(amount) AS collection FROM party_collection WHERE user_id IN (".$team.") AND entry_date >= '".$_REQUEST['fdate']."' AND entry_date <= '".$_REQUEST['tdate']."'"));
				...
				$shorfall = $row_tar['target_val']-$resp;
			?>
            	<tr>
                	<td><?=$row_tar['prod_code']?></td>
                    <td><?=$row_tar['task_name']?></td>
                    <?php /*?><td><?=$row_tar['remark']?></td><?php */?>
                    <td style="text-align:right"><?=$row_tar['target_val']?></td>
                    ...
                    <td style="text-align:right"><?php if($row_tar['target_val']){$perc = number_format((($resp/$row_tar['target_val'])*100),'2','.',''); echo $perc."%";}else{ echo $perc=0;}?></td>
                 </tr>
			<?php
				$ach_per1 += $perc;
			}
		...
		<?php
			...
			if($bar[2]=="1"){ 
				$top_perf[]=$row_mng["name"]." | ".$row_mng["oth_empid"]." | ".$row_mng["username"];
			}else if($bar[2]=="2"){
				$med_perf[]=$row_mng["name"]." | ".$row_mng["oth_empid"]." | ".$row_mng["username"];
			}else if($bar[2]=="3"){
				$low_perf[]=$row_mng["name"]." | ".$row_mng["oth_empid"]." | ".$row_mng["username"];
			}else{
				$non_perf[]=$row_mng["name"]." | ".$row_mng["oth_empid"]." | ".$row_mng["username"];
			}
            ?>
        </td>
    	...
    	<div id="collapse-<?=$i?>" class="collapse" data-parent="#accordion" aria-labelledby="heading-<?=$i?>">
      		<div class="card-body">
            	<?php
				$res_ch1 = mysqli_query($link1,"SELECT username, name, oth_empid, designationid FROM admin_users WHERE reporting_manager='".$row_mng["username"]."'");
		...
		<?php
			...
			///// make array count performance wise
			if($bar[2]=="1"){ 
				$top_perf[]=$row_ch1["name"]." | ".$row_ch1["oth_empid"]." | ".$row_ch1["username"];
			}else if($bar[2]=="2"){
				$med_perf[]=$row_ch1["name"]." | ".$row_ch1["oth_empid"]." | ".$row_ch1["username"];
			}else if($bar[2]=="3"){
				$low_perf[]=$row_ch1["name"]." | ".$row_ch1["oth_empid"]." | ".$row_ch1["username"];
			}else{
				$non_perf[]=$row_ch1["name"]." | ".$row_ch1["oth_empid"]." | ".$row_ch1["username"];
			}
            ?>
                ...
                <div class="progress-bar progress-bar-<?=$bar[0]?> progress-bar-striped" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width:<?=$perf?>%" title="<?=$perf?>% Achieve">
                  <?=$perf?>% <?=$bar[1]?>
            			...
            			<div id="collapse-<?=$i?>-<?=$j?>" class="collapse" data-parent="#accordion-<?=$i?>" aria-labelledby="heading-<?=$i?>-<?=$j?>">
              				<div class="card-body">
                                <?php
								$res_ch2 = mysqli_query($link1,"SELECT username, name, oth_empid, designationid FROM admin_users WHERE reporting_manager='".$row_ch1["username"]."'");
		...
		<?php
			...
			///// make array count performance wise
			if($bar[2]=="1"){ 
				$top_perf[]=$row_ch2["name"]." | ".$row_ch2["oth_empid"]." | ".$row_ch2["username"];
			}else if($bar[2]=="2"){
				$med_perf[]=$row_ch2["name"]." | ".$row_ch2["oth_empid"]." | ".$row_ch2["username"];
			}else if($bar[2]=="3"){
				$low_perf[]=$row_ch2["name"]." | ".$row_ch2["oth_empid"]." | ".$row_ch2["username"];
			}else{
				$non_perf[]=$row_ch2["name"]." | ".$row_ch2["oth_empid"]." | ".$row_ch2["username"];
			}
            ?>
                ...
                <div class="progress-bar progress-bar-<?=$bar[0]?> progress-bar-striped" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width:<?=$perf?>%" title="<?=$perf?>% Achieve">
                  <?=$perf?>% <?=$bar[1]?>
                      					...
                      					<div id="collapse-<?=$i?>-<?=$j?>-<?=$k?>" class="collapse" data-parent="#accordion-<?=$i?>" aria-labelledby="heading-<?=$i?>-<?=$j?>-<?=$k?>">
                          					<div class="card-body">
                                                <?php
												$res_ch3 = mysqli_query($link1,"SELECT username, name, oth_empid, designationid FROM admin_users WHERE reporting_manager='".$row_ch2["username"]."'");
                    ...
                    <?php }?>
													...
													<td style="text-align:right"><?php if($shorfall>0){ echo round($shorfall);}?></td>
													<td style="text-align:right"><?php if($row_tar['target_val']){$perc = number_format((($resp/$row_tar['target_val'])*100),'2','.',''); echo $perc."%";}else{ echo $perc="NA";}?></td>
												 </tr>
											<?php
											$ach_per4 += $perc;
											}
		...
		<?php
			...
			///// make array count performance wise
			if($bar[2]=="1"){ 
				$top_perf[]=$row_ch3["name"]." | ".$row_ch3["oth_empid"]." | ".$row_ch3["username"];
			}else if($bar[2]=="2"){
				$med_perf[]=$row_ch3["name"]." | ".$row_ch3["oth_empid"]." | ".$row_ch3["username"];
			}else if($bar[2]=="3"){
				$low_perf[]=$row_ch3["name"]." | ".$row_ch3["oth_empid"]." | ".$row_ch3["username"];
			}else{
				$non_perf[]=$row_ch3["name"]." | ".$row_ch3["oth_empid"]." | ".$row_ch3["username"];
			}
            ?>
