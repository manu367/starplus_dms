...
function getAcheivement($taskType,$fromDate,$toDate,$prodCode,$saleType,$team,$link1){
	if($taskType == "Dealer Visit"){
		...
		$resp = $p + $s;*/
		$row_totcnt =  mysqli_fetch_assoc(mysqli_query($link1,"SELECT COUNT(id) AS totdealer FROM dealer_visit WHERE userid IN (".$team.") AND visit_date >= '".$fromDate."' AND visit_date <= '".$toDate."'"));
		$resp = $row_totcnt["totdealer"];
	}
		...
		if($saleType=="P" || $saleType==""){
			////////// exclude combo
			$row_so_pri =  mysqli_fetch_assoc(mysqli_query($link1,"SELECT SUM(req_qty) AS socnt FROM purchase_order_data WHERE prod_cat!='C' AND po_no IN (SELECT po_no FROM purchase_order_master WHERE sale_type LIKE 'PRIMARY' AND create_by IN (".$team.") AND entry_date >= '".$fromDate."' AND entry_date <= '".$toDate."' AND dispatch_challan!='' AND status IN ('Processed')) AND prod_code IN (SELECT productcode FROM product_master WHERE productsubcat IN (SELECT psubcatid FROM product_sub_category WHERE prod_sub_cat='".$prodCode."'))"));
		//if($row_so["socnt"]){ echo $ach = $row_so["socnt"];}else{echo $ach = 0;}
			//$o = $row_so_pri["socnt"];
			////////// include combo
			$row_so_pricombo =  mysqli_fetch_assoc(mysqli_query($link1,"SELECT SUM(b.bom_qty*a.req_qty) AS csocnt FROM purchase_order_data a, combo_master b, purchase_order_master c WHERE a.po_no=c.po_no AND c.sale_type LIKE 'PRIMARY' AND c.create_by IN (".$team.") AND c.entry_date >= '".$fromDate."' AND c.entry_date <= '".$toDate."' AND c.dispatch_challan!='' AND c.status IN ('Processed') and a.prod_cat='C' and a.prod_code=b.bom_modelcode AND b.status='1' AND b.bom_partcode IN (SELECT productcode FROM product_master WHERE productsubcat IN (SELECT psubcatid FROM product_sub_category WHERE prod_sub_cat='".$prodCode."'))"));
			$o = $row_so_pri["socnt"] + $row_so_pricombo["csocnt"]; 
		}
		...
		if($saleType=="S" || $saleType==""){
			...
			$row_so_seccombo =  mysqli_fetch_assoc(mysqli_query($link1,"SELECT SUM(b.bom_qty*a.req_qty) AS csocnt FROM purchase_order_data a, combo_master b, purchase_order_master c WHERE a.po_no=c.po_no AND c.sale_type LIKE 'SECONDARY' AND c.create_by IN (".$team.") AND c.entry_date >= '".$fromDate."' AND c.entry_date <= '".$toDate."' AND c.status IN ('Approved','Processed') and a.prod_cat='C' and a.prod_code=b.bom_modelcode AND b.status='1' AND b.bom_partcode IN (SELECT productcode FROM product_master WHERE productsubcat IN (SELECT psubcatid FROM product_sub_category WHERE prod_sub_cat='".$prodCode."'))"));
			
			$n = $row_so_sec["socnt"] + $row_so_seccombo["csocnt"];
		}
    ...
    <div class="col-sm-9 tab-pane fade in active" id="home">
     <h2 align="center"><i class="fa fa-bullseye"></i> Sales Force Dashboard</h2>
      ...
      <div class="row">
            ...
            <div class="col-sm-3 col-md-3 col-lg-3"><label class="col-md-9">Employee Name</label>
                <select name="user_id" id="user_id" class="form-control selectpicker" data-live-search="true" >
						...
						if($_SESSION["userid"]=="admin"){
							$sql = mysqli_query($link1, "SELECT username,name,oth_empid FROM admin_users where 1 AND status='Active' and oth_empid!='' AND status='Active' order by name");
						}else{
							$sql = mysqli_query($link1, "SELECT username,name,oth_empid FROM admin_users where 1 AND (reporting_manager='".$_SESSION["userid"]."' or username='".$_SESSION["userid"]."') AND status='Active' order by name");
						}
            ...
            <div class="col-sm-2 col-md-2 col-lg-2"><label class="col-md-6">Type</label>
                <select name="sale_type" id="sale_type" class="form-control">
                    ...
                    <option value="S"<?php if ($_REQUEST['sale_type'] == "S") { echo "selected";}?>>Secondary</option>
                </select>
            </div>
            <div class="col-sm-1 col-md-1 col-lg-1"><label class="col-md-6">&nbsp;</label><br/>
...
<div id="accordion">
	<div class="card">
		...
		<?php
			...
			//// get emp performance
			$res_empperf = mysqli_query($link1,"SELECT * FROM emp_performance WHERE username='".$row_mng["username"]."' AND year='".date('Y', strtotime($_POST['fdate']))."' AND month='".date('n', strtotime($_POST['fdate']))."'");
						...
						<table id="myTable2" class="dt_table" width="100%" border="0" cellpadding="2"  cellspacing="0">
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td style="text-align:right"><?=round($resp)?>
                                    <?php if($row_tar['task_name']=="Sale Order"){?>
                    <a href="../admin/excelexport.php?rname=<?=base64_encode("masterdashsaleorder")?>&rheader=<?=base64_encode("Sales Order")?>&fdate=<?=base64_encode($_POST['fdate'])?>&tdate=<?=base64_encode($_POST['tdate'])?>&userid=<?=base64_encode($row_ch1["username"])?>&psc=<?=base64_encode($row_tar['prod_code'])?>&sale_type=<?=base64_encode($_POST['sale_type'])?>" style="float: left;" title="Export details in excel" class="text-success"><i class="fa fa-file-excel-o fa-lg" title="Export details in excel"></i></a>
                    <?php }else if($row_tar['task_name']=="Dealer Visit"){?>
		...
		<?php
			...
			//// get emp performance
			$res_empperf = mysqli_query($link1,"SELECT * FROM emp_performance WHERE username='".$row_ch1["username"]."' AND year='".date('Y', strtotime($_POST['fdate']))."' AND month='".date('n', strtotime($_POST['fdate']))."'");
			$row_empperf = mysqli_fetch_assoc($res_empperf);
                                        ...
                                        <div class="card-body">
											...
											?>
												<tr>
													...
													<td style="text-align:right"><?=round($resp)?>
                                                    <?php if($row_tar['task_name']=="Sale Order"){?>
                    <a href="../admin/excelexport.php?rname=<?=base64_encode("masterdashsaleorder")?>&rheader=<?=base64_encode("Sales Order")?>&fdate=<?=base64_encode($_POST['fdate'])?>&tdate=<?=base64_encode($_POST['tdate'])?>&userid=<?=base64_encode($row_ch2["username"])?>&psc=<?=base64_encode($row_tar['prod_code'])?>&sale_type=<?=base64_encode($_POST['sale_type'])?>" style="float: left;" title="Export details in excel" class="text-success"><i class="fa fa-file-excel-o fa-lg" title="Export details in excel"></i></a>
                    <?php }else if($row_tar['task_name']=="Dealer Visit"){?>
		...
		<?php
			...
			//// get emp performance
			$res_empperf = mysqli_query($link1,"SELECT * FROM emp_performance WHERE username='".$row_ch2["username"]."' AND year='".date('Y', strtotime($_POST['fdate']))."' AND month='".date('n', strtotime($_POST['fdate']))."'");
			$row_empperf = mysqli_fetch_assoc($res_empperf);
                                        ...
                                        <?php
											...
											?>
												<tr>
													...
													<td style="text-align:right"><?=round($resp)?>
                                                    <?php if($row_tar['task_name']=="Sale Order"){?>
                    <a href="../admin/excelexport.php?rname=<?=base64_encode("masterdashsaleorder")?>&rheader=<?=base64_encode("Sales Order")?>&fdate=<?=base64_encode($_POST['fdate'])?>&tdate=<?=base64_encode($_POST['tdate'])?>&userid=<?=base64_encode($row_ch3["username"])?>&psc=<?=base64_encode($row_tar['prod_code'])?>&sale_type=<?=base64_encode($_POST['sale_type'])?>" style="float: left;" title="Export details in excel" class="text-success"><i class="fa fa-file-excel-o fa-lg" title="Export details in excel"></i></a>
                    <?php }else if($row_tar['task_name']=="Dealer Visit"){?>
		...
		<?php
			...
			//// get emp performance
			$res_empperf = mysqli_query($link1,"SELECT * FROM emp_performance WHERE username='".$row_ch3["username"]."' AND year='".date('Y', strtotime($_POST['fdate']))."' AND month='".date('n', strtotime($_POST['fdate']))."'");
			$row_empperf = mysqli_fetch_assoc($res_empperf);
