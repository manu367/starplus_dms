...
if($_POST['upd']=='Update'){
	...
	$flag=dailyActivity($_SESSION['userid'],$docid,"PO","EDIT",$ip,$link1,$flag);
	if ($flag) {
        mysqli_commit($link1);
...
<div class="container-fluid">
 <div class="row content">
    ...
    ?>
   <div class="col-sm-9">
      <h2 align="center"><i class="fa fa-shopping-basket"></i>&nbsp;&nbsp;Edit Purchase Order </h2><br/>
                ...
                <td class="col-md-2" align="left">
                	<select name="prod_code[<?php echo $i;?>]" id="prod_code[<?php echo $i;?>]" class="form-control selectpicker" required data-live-search="true" onChange="checkDuplicate('<?php echo $i;?>', this.value);" style="width:200px">
                        ...
                        while ($br = mysqli_fetch_array($check1)) {
                            ?>
                            <option value="<?php echo $br['productcode'];?>"<?php if($br['productcode']==$podata_row['prod_code']){ echo "selected";}?>><?= $br['productname'] . ' | ' . $br['productcolor'] . ' | ' . $br['productcode']; ?></option>
                        <?php } ?>
                    </select>
                    </td>
  ...
  <div class="row" align="center">
   <input type="submit" class="btn<?=$btncolor?>" name="upd" id="upd" value="Update">&nbsp;&nbsp;&nbsp;
              <input title="Back" type="button" class="btn<?=$btncolor?>" value="Back" onClick="window.location.href='purchaseOrderList.php?<?=$pagenav?>'">
  </div>
  <br><br>
  </form>
 </div><!--close col-sm-9-->
