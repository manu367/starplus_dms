...
/////// get to party state
$to_state = getLocationDetails($_REQUEST["po_from"],"state",$link1);
@extract($_POST);
...
if ($_POST) {
    if ($_POST['upd'] == 'Save') {
		...
		$sessionMessageIdent = isset($_SESSION['messageIdentLPC'])?$_SESSION['messageIdentLPC']:'';
		if($messageIdent!=$sessionMessageIdent){//if its different:
		//save the session var:
		$_SESSION['messageIdentLPC'] = $messageIdent;
		...
		if (mysqli_num_rows(mysqli_query($link1,"SELECT id FROM billing_master WHERE inv_ref_no='".$invoiceno."' AND from_location='".$partycode."' AND status!='Cancelled'"))>0) {
			$cflag="danger";
			$cmsg="Failed";
			$msg = "Request could not be processed. Please try again. You have entered duplicate invoice number. ".$invoiceno;
        ...
        //// Make System generated PO no.//////
  		$res_po = mysqli_query($link1, "select max(temp_no) as no from vendor_order_master where po_to='".$parentcode."' AND req_type='LPC'");
  		$row_po=mysqli_fetch_assoc($res_po);
      	$c_nos = $row_po[no]+1;
        $po_no = $parentcode . "LPC" . $c_nos;
        mysqli_autocommit($link1, false);
        $flag = true;
		$err_msg="";
		$tax_total = $tot_sgst_amt+$tot_cgst_amt+$tot_igst_amt;
        ///// Insert Master Data
       $query1 = "INSERT INTO vendor_order_master set po_to='" . $parentcode . "',po_from='" . $partycode . "',po_no='" . $po_no . "',temp_no='" . $c_nos . "',ref_no='" . $ref_no . "',requested_date='" . $today . "',entry_date='" . $today . "',entry_time ='" . $currtime . "',req_type='LPC',status='Pending',po_value='" . $sub_total . "',create_by='" . $_SESSION['userid'] . "',ip='" . $ip . "',taxtype='" . $tax_type . "',taxper='" . $tax_per . "',taxamount='" . $tax_total . "',currency_type='" . $currency_type . "',invoice_no='" . $invoiceno . "',invoice_date='" . $invoicedate . "',remark='" . $remark . "',delivery_address='" . $delivery_address . "',round_off='" . $round_off . "',total_sgst_amt='" . $tot_sgst_amt . "',total_cgst_amt='" . $tot_cgst_amt . "',total_igst_amt='" . $tot_igst_amt . "',total_amt='" . $grand_total . "',tcs_per='".$tcs_per."', tcs_amt='".$tcs_amt."',grand_total='".$final_total."',ledger_name='".$ledgername."', document_type='".$doctype."',freight='".$freight."',tds='".$tds_194q."'";
        ...
        mysqli_close($link1);
			...
			$msg = "Request could not be processed. You have entered duplicate invoice no.".$invoiceno;
		}
		}else {
    ...
    <body onKeyPress="return keyPressed(event);">
        <div class="container-fluid">
            <div class="row content">
                ...
                ?>
                <div class="col-sm-9">
                    <h2 align="center"><i class="fa fa-ship"></i> Add New Local Purchase From Customer</h2><br/>
                                ...
                                <div class="col-md-10"><label class="col-md-5 control-label">Purchase From<span style="color:#F00">*</span></label>
                                    <div class="col-md-7">
                                        <select name="po_from" id="po_from" required class="form-control selectpicker required" data-live-search="true" onChange="document.frm1.submit();">
                                            ...
                                            <?php
                                            $sql_parent = "select uid,location_id from access_location where uid='" . $_SESSION['userid'] . "' and status='Y' AND id_type NOT IN ('HO','BR')";
                        ...
                        <form id="frm2" name="frm2" class="form-horizontal" action="" method="post">
                                ...
                                <table width="100%" id="itemsTable1" class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        <tr id='addr0'>
                                            ...
                                            <td class="col-md-1"><input type="text" class="form-control number required" name="price[0]" id="price[0]" onKeyUp="rowTotal(0);" autocomplete="off" required style="text-align: right;padding: 3px; width:100px;" min="1"></td>                                            
                            ...
                            <div class="form-group">
                                <div class="col-md-12" align="center">
                                    ...
                                    <input type="hidden" name="currency_type" id="currency_type" value="INR"/>
