...
@extract($_POST);
	if($_POST){
		...
		$messageIdent = md5($_POST['postinv_no'] . $_POST['po_no']);
		...
		if($messageIdent!=$sessionMessageIdent){//if its different:
			...
			$allowed1 = array('gif', 'png', 'jpg', 'jpeg', 'pdf', 'PDF', 'xlsx', 'xls');
			...
			$res_dupl = mysqli_query($link1,"SELECT id FROM billing_master WHERE inv_ref_no='".$_POST['postinv_no']."' AND from_location='".$_POST['po_to']."' AND status!='Cancelled'")or die(mysqli_error($link1));
			//// check post invoice no. is already entered
			if(mysqli_num_rows($res_dupl)>0){
				$cflag="danger";
				$cmsg="Failed";
			...
			while($row_grnData=mysqli_fetch_assoc($res_grnData)){
				...
				$totqty=$_POST[$bill_okqty]+$_POST[$bill_damageqty]+$_POST[$bill_missqty]+$_POST[$bill_excessqty];
				/////////				
				if($totqty!=""){
					if($totqty !=0 && $totqty !="" && $totqty !=0.00){	
						//// insert grn data 	
					   	$value = $totqty *  $_POST[$bill_price];
						...
						///// sum of  ok, damge , missing qty and minus from from pending qty //////////////
					 	$sum = $okqty+$_POST[$bill_missqty]+$_POST[$bill_damageqty];			   
						...
						if (!$upd_spd) {
							$flag = false;
							$error_msg = "Error detailsVOD: " . mysqli_error($link1) . ".";
						}
						//// check if query is not executed				
					
							//////////  update inventory  start  ////////////////////////////		
							if(mysqli_num_rows(mysqli_query($link1,"select partcode from stock_status where partcode='".$row_grnData["prod_code"]."' and asc_code='".$po_from."' and sub_location='".$stock_in."'"))>0){
								$result=mysqli_query($link1,"update stock_status set qty= qty+'".$okqty."' , okqty=okqty+'".$okqty."',broken=broken+'".$_POST[$bill_damageqty]."',missing=missing+'".$_POST[$bill_missqty]."',updatedate='".$datetime."' where partcode='".$row_grnData["prod_code"]."' and asc_code='".$po_from."' and sub_location='".$stock_in."'");
							}
							else{
								//// if product is not exist then add in inventory
								$result=mysqli_query($link1,"insert into stock_status set asc_code='".$po_from."',sub_location='".$stock_in."',partcode='".$row_grnData["prod_code"]."',qty = '".$okqty."' , okqty='".$okqty."',broken='".$_POST[$bill_damageqty]."',missing='".$_POST[$bill_missqty]."',updatedate='".$datetime."'");
							}
							//// check if query is not executed
							if (!$result) {
						 		$flag = false;
							...
							///////////  entry in stock ledger///////////////////////////////////////////////////////////
					 		if($okqty!=0 && $okqty!="" && $okqty!=0.00){					
								$flag=stockLedger($grnno,$today,$row_grnData["prod_code"],$po_to,$stock_in,$stock_in,"IN","OK","Receive Against GRN",$okqty,$_POST[$bill_price],$_SESSION['userid'],$today,$currtime,$ip,$link1,$flag); 
						
							}
							if($_POST[$bill_damageqty]!=0 && $_POST[$bill_damageqty]!="" && $_POST[$bill_damageqty]!=0.00){
								$flag=stockLedger($grnno,$today,$row_grnData["prod_code"],$po_to,$stock_in,$stock_in ,"IN","DAMAGE","Receive Against GRN",$_POST[$bill_damageqty],$_POST[$bill_price],$_SESSION['userid'],$today,$currtime,$ip,$link1,$flag);
							}
							if($_POST[$bill_missqty]!=0 && $_POST[$bill_missqty]!="" && $_POST[$bill_missqty]!=0.00){
					  			$flag=stockLedger($grnno,$today,$row_grnData["prod_code"],$po_to,$stock_in,$stock_in ,"IN","MISSING","Receive Against GRN",$_POST[$bill_missqty],$_POST[$bill_price],$_SESSION['userid'],$today,$currtime,$ip,$link1,$flag);
							}
			...
			/////////////////////////////// insert data into grn master  table///////////////////////////////////////////////
		 	$grn_master="insert into billing_master set from_location ='".$_POST['po_to']."', to_location='".$_POST['po_from']."',sub_location='".$_POST['stock_in']."',from_gst_no='".$vend_det[8]."', from_partyname='".$vend_det[0]."', party_name='".$loc_det[0]."', to_gst_no='".$loc_det[10]."' ,ref_no='".$_POST['po_no']."', receive_date='".$today."' ,receive_time='".$time."', entry_date ='".$today."', entry_time='".$currtime."',entry_by='".$_SESSION["userid"]."' , status='".$status."' , challan_no='".$grnno."', basic_cost='".$basic_cost."', tax_cost='".$toaxtot."',total_sgst_amt='".$tot_sgst."',total_cgst_amt='".$tot_cgst."',total_igst_amt='".$tot_igst."',round_off='".$round_off."', total_cost='".$grandtot."',tcs_per='".$tcs_per."', tcs_amt='".$tcs_amt."' , inv_ref_no='".$_POST['postinv_no']."',po_inv_date='".$_POST['postinv_date']."', receive_remark='".$_POST['rcv_rmk']."',type='GRN',document_type='INVOICE', sale_date = '".$today."' ,grn_doc = '".$path1."',pod1='".$pod1_path."',pod2='".$pod2_path."',from_state='".$vend_det[2]."', to_state='".$loc_det[7]."', from_city='".$vend_det[1]."', to_city='".$loc_det[6]."', from_pincode='".$vend_det[6]."', to_pincode='".$loc_det[9]."', from_phone='".$vend_det[4]."', to_phone='".$loc_det[3]."', from_email='".$vend_det[5]."', to_email='".$loc_det[2]."', from_addrs='".$vend_det[3]."', disp_addrs='".$vend_det[3]."', to_addrs='".$loc_det[4]."', deliv_addrs='".$po_row["delivery_address"]."',freight='".$freight."',tds='".$tds_194q."'";
			...
			////// check  amnd update status///////////////		
			 ...
			 if($sel_pend_qty['pend_qty']==0.00){ 
				 $res2 = mysqli_query($link1,"update vendor_order_master set status='Received',sub_location='".$_POST['stock_in']."'  where po_no='".$_POST['po_no']."'");       
				 //// check if query is not executed
				if (!$res2) {
					$flag = false;
					$error_msg = "Error detailsVOM: " . mysqli_error($link1) . ".";
				}
			  } 
		 ...
		 $res_ac_ledger = mysqli_query($link1,"INSERT INTO party_ledger set location_code='".$po_from."',entry_date='".$today."', doc_type = 'GRN',doc_no='".$grnno."',doc_date='".$today."',cr_dr='CR',amount='".$_POST['grand_total']."' , cust_id = '".$po_to."' ");
			...
			$resp = explode("~",storeLedgerTransaction($_POST['po_from'],$grnno,$today,"1","Purchase",$arr_tax,$arr_val,$tcs_per,$tcs_amt,$round_off,$gst_type,$arr_ldg_name,"Direct Purchase","Purchase Accounts",$link1,$flag));
			$flag = $resp[0];
			$error_msg = $resp[1];
	...
	function recalculatetotal(){
		...
		function calculateTotal(){
			...
			for(var i=1; i < maxrow; i++){
				...
				if(document.getElementById("damage_qty"+i).value==""){ var dmqtynew=0; }else{ var dmqtynew=parseInt(document.getElementById("damage_qty"+i).value);}
				if(document.getElementById("miss_qty"+i).value==""){ var msqtynew=0; }else{ var msqtynew=parseInt(document.getElementById("miss_qty"+i).value);}
			...
			document.getElementById("totdm_qty").value = newdmqty;
			document.getElementById("totms_qty").value = newmsqty;
			document.getElementById("totex_qty").value = newexqty;
		}
		function putdata(){			
			  ...
			  <table class="table table-bordered" width="100%">
				<tbody>
				  ...
				  <tr>
					...
					<td><label class="control-label">Invoice Number<span style="color:#F00">*</span></label></td>
					<td><input type="text" name="invoice_no" id="invoice_no" value="<?=$po_row['invoice_no']?>" class="form-control required" required onKeyUp="putdata();"/></td>
				  </tr>
				  <tr>
				    <td><label class="control-label">Invoice Date<span style="color:#F00">*</span></label></td>
				    <td><input type="text" class="form-control required span2" required name="invoicedate" id="invoicedate" autocomplete="off" onChange="putdata();"></td>
				    ...
				    <td><select name="stock_inn" id="stock_inn" required class="form-control selectpicker required" data-live-search="true" onChange="putdata();">
											...
											?>
											<option value="<?=$smf_row['sub_location']?>" <?php if($smf_row['sub_location']==$_REQUEST['stock_inn'])echo "selected";?>><?=$smf_row['sub_location_name']." | ".$smf_row['sub_location']?></option>
											<?php
											}
											?>
                                        </select></td>
			...
			<table class="table table-bordered" width="200%">
				...
				while($data_row = mysqli_fetch_assoc($data_res)){
				 ...
				 ?>
					<tr>
                        ...
                        <td><input name="inv_qty<?=$data_row['id']?>" onBlur="checkPriceInvRcb(<?=$i?>);rowTotal('<?=$i?>');" id="inv_qty<?=$i?>" type="text" class="form-control digits required" style="width:80px;text-align:right" required/></td>
				  		...
				  		<td><input type="text" class="digits form-control required" style="width:80px;" name="ok_qty<?=$data_row['id']?>" id="ok_qty<?=$i?>"  autocomplete="off" onBlur="checkPriceInvRcb(<?=$i?>);" onKeyUp="checkRecQty('<?=$i?>');" value="<?=$data_row['qty'];?>"></td>
				...
				?>
					<tr>
						...
						<td><input type="text" name="totok_qty" id="totok_qty"  class="form-control" readonly></td>
                        <td><input type="text" name="totdm_qty" id="totdm_qty"  class="form-control" readonly></td>
                        <td><input type="text" name="totms_qty" id="totms_qty"  class="form-control" readonly></td>
                        <td><input type="text" name="totex_qty" id="totex_qty"  class="form-control" readonly></td>
			...
			<table class="table table-bordered" width="100%">
				<tbody>          
                   ...
                   <tr>
					 ...
					 <td><input type="text" name="tds_194q" id="tds_194q" class="form-control number" value="0.00" style="width:150px;text-align:right"/></td>
				   </tr>
				   <tr>
				   ...
				   <tr>
					...
					<input type="submit" class="btn<?=$btncolor?>" name="upd" id="upd" value="Receive" title="Receive" <?php if($_POST['upd']=="Receive"){ echo "disabled";}?>>&nbsp;
