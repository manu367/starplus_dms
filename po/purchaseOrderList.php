...
//// get cancel rights
$isCnlRight = getCancelRightNew($_SESSION['userid'],"7",$link1);
////// filters value/////
$filter_str = "";
      ...
      <div class="form-group"  id="page-wrap" style="margin-left:10px;"><br/><br/>
       <table  width="98%" id="myTable" class="table-striped table-bordered table-hover" align="center">
          ...
          <tbody>
			...
			$sno=0;
			if($_SESSION["userid"]=="admin"){
				$sql=mysqli_query($link1,"Select * from purchase_order_master where 1 ".$filter_str." ".$statt." order by id desc");
			}else{
				$sql=mysqli_query($link1,"Select * from purchase_order_master where po_from in (".$accesslocation.") AND po_no IN (SELECT po_no FROM purchase_order_data WHERE psc_id IN (".$accesspsc.") AND brand_id IN (".$accessbrand.")) ".$filter_str." ".$statt." order by id desc");
			}
            ...
            <tr class="even pointer">
              <td><?php echo $sno;?></td>
              <td><?php echo str_replace("~",",",getLocationDetails($row['po_to'],"name,city,state,asc_code",$link1));?></td>
              <td><?php echo str_replace("~",",",getLocationDetails($row['po_from'],"name,city,state,asc_code",$link1));?></td>
              ...
              <td align="center"><a href='../print/print_po.php?rb=view&id=<?php echo base64_encode($row['po_no']);?><?=$pagenav?>' target="_blank"  title='Print PO'><i class="fa fa-print fa-lg" title="Print PO"></i></a></td>
			  <td align="center"><a href='poDetails.php?op=edit&id=<?php echo base64_encode($row['po_no']);?><?=$pagenav?>'  title='view'><i class="fa fa-eye fa-lg" title="view details"></i></a></td>
			 
              <td align="center"> <?php if ($row['status']=="PFA"){?><a href='editNewPO.php?op=edit&id=<?php echo base64_encode($row['po_no']);?><?=$pagenav?>'  title='edit'><i class="fa fa-edit fa-lg" title="edit details"></i></a><?php }?></td>
			 
			   <td align="center">	<?php 
	 if($isCnlRight== 1){  if ($row['status']=="Approved" || $row['status']=="PFA" ) { ?><a href='cancelNewPO.php?op=cancel&id=<?php echo base64_encode($row['po_no']);?><?=$pagenav?>'  title='Cancel PO'><i class="fa fa-trash fa-lg" title="Cancel PO"></i></a><?php } }?>
