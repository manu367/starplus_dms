...
<head>
	...
	function rowTotal(ind){
		...
		if(vend_state == loc_state){
			...
			document.getElementById(cgstamt).value=(cgstamtval);
	 
			var sgstamtval = ((total)*(document.getElementById(sgst_per).value))/100;
			document.getElementById(sgstamt).value=(sgstamtval);	 
			
			document.getElementById(linetotal).value = parseFloat(total+cgstamtval+sgstamtval);
			recalculatetotal();
		}
		else {
			var igstamtval = ((total)*(document.getElementById(igst_per).value))/100;
			document.getElementById(igstamt).value=(igstamtval);
			
			document.getElementById(linetotal).value = (total+igstamtval);
			recalculatetotal();
		} 
		//recalculatetotal(); 
	}
	...
	function recalculatetotal(){
		...
		var loc_state = '<?=$loc_det[7]?>';
			...
			document.getElementById("sub_total").value=(sum_subtot);
			document.getElementById("grand_total").value=(sum_total);
			document.getElementById("tax_total").value=(sum_cgsttot+sum_sgsttot+sum_igsttot);
		}
