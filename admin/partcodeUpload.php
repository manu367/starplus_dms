	...
	if($_POST['Submit'] == "Upload") {
		...
		////// retrive data from excel
      	for($row = 2; $row <= $highest; $row++) {
			...
			$serial_len = $sheet->getCellByColumnAndRow(24, $row)->getValue();
			if(mysqli_num_rows(mysqli_query($link1,"SELECT id FROM product_master WHERE productname='".$productname."'"))==0){
				////// product code create
				$ref = mysqli_query($link1,"SELECT COUNT(id) AS cnt FROM product_master");
				$row_part = mysqli_fetch_assoc($ref);
				$result = $row_part['cnt']+1;
				$pad = str_pad($result,5,"0",STR_PAD_LEFT);  
				$reference = "P".$pad;
				/////// insert in table
				$res_part = mysqli_query($link1, "INSERT INTO product_master SET productname='".$productname."',productcode='".$reference."',sap_code='".$reference."',productcolor='".$productcolor."',productdesc='".$productdesc."',model_name='".$modelname."',productcategory='".$productcategory."',brand='".$brand."',status='Active',warranty_days='".$wsdays."',warranty_terms='".$wsterm."',grace_period='".$storageperiod."',eol='',weight='".$grossweight."',net_weight='".$netweight."',scrap_weight='".$scrapweight."',pro_rata='".$prorata."',battery_rating='".$batteryrating."',division='".$division."',createdate='".$datetime."',type_id='".$producttype."',hsn_code='".$hsncode."' ,productsubcat = '".$productsubcategory."', is_serialize = '".$is_serialized."',serial_length='".$serial_len."' , product_code = '".$prodcode."', product_code2='' , pc_serial_place = '', model_code='".$modelcode."', model_code2='', mc_serial_place='',other_specification1='".$othspec1."',other_specification2='".$othspec2."'");
				...
				$flag = dailyActivity($_SESSION['userid'],$reference,"PRODUCT","UPLOAD",$ip,$link1,$flag);
			}
		}
		if($flag){
			mysqli_commit($link1);
