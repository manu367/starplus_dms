...
if($_POST){
	if ($_POST['add']=='ADD'){
		...
		if($num_totusr>=$num_usrlimit){
			$msg.="You have reached your user limit.<br/>";
		}else{
   			///// check if email id , mobile no. and emp id should be unique
			
   			$res_exist = mysqli_query($link1,"SELECT uid, oth_empid, phone, emailid FROM admin_users WHERE oth_empid='".$emp_id."' OR phone='".$phone."' OR emailid='".$email."'")or die("error1".mysqli_error($link1));
   			if(mysqli_num_rows($res_exist)==0){
				...
				///// start update auto right to this new user by access role tab written on 8 feb 2024 by shekhar
				$utypename = getAnyDetails($u_type,"typename","refid","usertype_master",$link1);
				///// get all assigend tabs
				$res_tab = mysqli_query($link1,"SELECT * FROM access_role_tab WHERE role_id='".$utypename."' AND status='1'");
				if(mysqli_num_rows($res_tab)>0){
					while($row_tab = mysqli_fetch_assoc($res_tab)){
						///// assign master/report/dashboard rights
						if($row_tab['tab_type']=="M"){
							$res_m = mysqli_query($link1,"INSERT INTO access_report SET uid='".$admiCode."', report_id='".$row_tab['tab_id']."',status='Y'")or die(mysqli_error($link1));
						}
						else if($row_tab['tab_type']=="P"){///// assign process rights
							$res_p = mysqli_query($link1,"INSERT INTO access_function SET uid='".$admiCode."', function_id='".$row_tab['tab_id']."',status='Y'")or die(mysqli_error($link1));
						}
						else{
						////// nothing to do
						}
				...
				///// end update auto right to this new user by access role tab written on 8 feb 2024 by shekhar
				////// return message
