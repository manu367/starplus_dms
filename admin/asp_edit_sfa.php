<?php
////// Function ID ///////
$fun_id = array("a"=>array(108)); // User:, Location:, Admin:22:
//////////////////////////
...
$arr_underuser = explode(",",$row_locdet["underuser"]);
...
if($_POST){
	if($_POST['Submit']=='Update'){
    	...
    	$sql="update asc_master set pwd='".$pswd."',name='".$locationname."',shop_name='".$locationname."',contact_person='".$contact_person."',landline='".$landline."',email='".$email."',phone='".$phone."',addrs='".$comm_address."',disp_addrs='".$del_addrs."',landmark='".$landmark."',city='".$locationcity."',pincode='".$pincode."',vat_no='".$tin_no."',pan_no='".$pan_no."',cst_no='".$cst_no."',st_no='".$st_no."',proprietor_type='".$proprietor."',tdsper='".$tdsper."',tcs_applicable='".$tcs_apl."',tcs_per='".$tcs_per."',account_holder='".$accountholder."',account_no='".$accountno."',bank_name='".$bankname."',bank_city='".$bankcity."',ifsc_code='".$ifsccode."',status='".$status."',update_date='".$datetime."', gstin_no='$_POST[gst_no]', segment='".$segment."',underuser='".implode(",",$underuser)."'  where  sno='".$refid."'";
    	mysqli_query($link1,$sql)or die("ER1".mysqli_error($link1));
		///// check mapping is already is there
	   if(mysqli_num_rows(mysqli_query($link1,"select mapped_code from mapped_master where uid='".$parentid."' and mapped_code='".$row_locdet["asc_code"]."'"))>0){
		  mysqli_query($link1,"update mapped_master set status='Y',update_date='".$today."' where uid='".$parentid."' and mapped_code='".$row_locdet["asc_code"]."'")or die("ER1".mysqli_error($link1));
	   }else{
		  mysqli_query($link1,"insert into mapped_master set mapped_code='".$row_locdet["asc_code"]."',uid='".$parentid."',status='Y',update_date='".$today."'")or die("ER2".mysqli_error($link1));
	   }
		...
		///// Update Location details if needed
    	$sql="update asc_master set update_date='".$datetime."', turnover ='".$turnover."'  , vision = '".$vision."', principal_place='".$principal_name."',principal_place_gstin='".$principal_name_gst."',principal_place_address='".$principal_name_address."' where  sno='".$refid."'";
    	mysqli_query($link1,$sql)or die("ER1".mysqli_error($link1));
		dailyActivity($_SESSION['userid'],$loccode,"LOCATION","UPDATE",$ip,$link1,"");
		////// return message
	...
	///// move to parent page
    //header("Location:asp_details_sfa.php?msg=".$msg."".$pagenav);
	//exit;
}
...
<div class="container-fluid">
  <div class="row content">
    ...
    ?>
    <div class="col-sm-9">
      <h2 align="center"><i class="fa fa-bank"></i> View/Edit Customer/Location (SFA)</h2>
      <h4 align="center">
          ...
          <br/>
          <span style="color:#FF0000"><?php echo $msg;?><br/>Kindly refersh the page or go back to see updated data.</span>
          <?php }
		  }
		   ?>
        </h4>
      <div class="form-group"  id="page-wrap" style="margin-left:10px;">
      	<ul class="nav nav-tabs">
            ...
            <li><a data-toggle="tab" href="#menu3"><i class="fa fa-upload"></i> Upload KYC Document</a></li>
            ...
            <div class="col-md-6"><label class="col-md-6 control-label">Segment <span class="red_small">*</span></label>
              <div class="col-md-6">
                  ...
                  <?php
					...
					?>
                	<option value="<?=$seg_row['segment']?>"<?php if($row_locdet['segment']==$seg_row['segment']){ echo "selected";}?>><?php echo $seg_row['segment']?></option>
                	<?php }?>
                </select>
              </div>
            </div>
          ...
          <div class="form-group">
            ...
            <div class="col-md-6"><label class="col-md-6 control-label">City <span class="red_small">*</span></label>
              <div class="col-md-6" id="citydiv">
                 <select  name='locationcity' id='locationcity' class='form-control required' required><option value=''>--Please Select--</option>
                 <?php 
		  ...
		  <div class="form-group">
            <div class="col-md-6"><label class="col-md-6 control-label">Email</label>
              <div class="col-md-6">
                <input name="email" type="email" class="email form-control" id="email" onBlur="return checkEmail(this.value,'email');" value="<?=$row_locdet['email']?>">
              </div>
            </div>
          ...
          <div class="form-group">
           <div class="col-md-6"><label class="col-md-6 control-label">Billing Address <span class="red_small">*</span></label>
              <div class="col-md-6">
                <textarea name="comm_address" id="comm_address" class="form-control required" rows="6" required onkeypress = " return ( (event.keyCode ? event.keyCode : event.which ? event.which : event.charCode)!= 13);"  onContextMenu="return false" style="resize:vertical"><?=$row_locdet['addrs']?></textarea>
              </div>
            </div>
            <div class="col-md-6"><label class="col-md-6 control-label">Delivery/Shipping Address</label>
              <div class="col-md-6">
               <textarea name="dd_address" id="dd_address" class="form-control" rows="6" onkeypress = " return ( (event.keyCode ? event.keyCode : event.which ? event.which : event.charCode)!= 13);"  onContextMenu="return false" style="resize:vertical"><?=$row_locdet['disp_addrs']?></textarea>
              </div>
            </div>
          </div>
		 <div class="form-group">
            ...
            <div class="col-md-6"><label class="col-md-6 control-label">Parent Location <span class="red_small">*</span></label>
              <div class="col-md-6" id="parentdiv">
                 <select name="parentid" id="parentid" required class="form-control required selectpicker"  data-live-search="true" >
                    <option value="">--Please Select--</option>
					<?php
	if($row_locdet['id_type']=="DL"){ $strr = "id_type='DS' AND state='".$row_locdet['state']."'"; }else if($row_locdet['id_type']=="DS"){ $strr = "id_type='HO'";}else{ $strr = "user_level<'".$row_locdet['user_level']."'";}
					$parent_query = "SELECT uid,asc_code,name,city,state FROM asc_master where ".$strr;
    $parent_res = mysqli_query($link1, $parent_query);
	
    while ($row_parent = mysqli_fetch_array($parent_res)) {
		$mapped_loc = mysqli_fetch_assoc(mysqli_query($link1,"SELECT uid FROM mapped_master WHERE mapped_code='".$row_locdet['asc_code']."' AND status='Y'"));
	?>
        <option value="<?=$row_parent['asc_code']?>"<?php if($row_parent['asc_code']==$mapped_loc["uid"]){ echo "selected";}?>><?php echo $row_parent['name']." , ".$row_parent['city']." , ".$row_parent['state']." , ".$row_parent['asc_code']?></option>
                <?php }?>
                 </select>
              </div>
            </div>
            ...
            <div class="col-md-6"><label class="col-md-6 control-label">TCS Applicable <span class="red_small">*</span></label>
              <div class="col-md-6">
                 <select name="tcs_applicable" id="tcs_applicable" required class="form-control required">
                    <option value="">--Please Select--</option>
                    <option value="N"<?php if($row_locdet['tcs_applicable']=="N"){ echo "selected";}?>>NO</option>
					 <option value="0.1"<?php if($row_locdet['tcs_per']=="0.1"){ echo "selected";}?>>0.1%</option>
                    <option value="1"<?php if($row_locdet['tcs_per']=="1.00"){ echo "selected";}?>>1%</option>
                 </select>
              </div>
            </div>
            <div class="col-md-6"><label class="col-md-6 control-label">Under User</label>
              <div class="col-md-6">
                ...
                <select name='underuser[]' id='underuser' class="form-control multiselect-ui" multiple="multiple">
				 ...
				 ?>
                 <option value="<?=$row_underuser["username"]?>"<?php if($array_underuser[$row_underuser['username']]=="Y"){ echo "selected";}?>><?=$row_underuser["name"]." | ".$row_underuser["username"]?></option>
                 <?php 
				 } 
				 ?>
                </select>
          ...
          <div class="form-group">
            <div class="col-md-12" align="center">
              ...
              <input title="Back" type="button" class="btn <?=$btncolor?>" value="Back" onClick="window.location.href='asp_details_sfa.php?<?=$pagenav?>'">
            </div>
          </div>
    </form>
    </div>
            <div id="menu2" class="tab-pane fade"> <br/>
              <form  name="frm2" id="frm2" class="form-horizontal" action="" method="post">
                  ...
                  <div class="form-group">
                    <div class="col-md-10"><label class="col-md-4 control-label">Principal Location Name</label>
                      <div class="col-md-6">
                        <input type="text" name="principal_name" class="form-control mastername" id="principal_name" value="<?php if(!empty($row_locdet['principal_place'])){ echo $row_locdet['principal_place'];}?>"/>
                      </div>
                    </div>
                  </div>
                  <div class="form-group">
                    <div class="col-md-10"><label class="col-md-4 control-label">Principal Location GSTIN</label>
                      <div class="col-md-6">
                        <input type="text" name="principal_name_gst" class="form-control alphanumeric" id="principal_name_gst" value="<?php if(!empty($row_locdet['principal_place_gstin'])){ echo $row_locdet['principal_place_gstin'];}?>" minlength="15" maxlength="15"/>
                      </div>
                    </div>
                  </div>
                  <div class="form-group">
                    <div class="col-md-10"><label class="col-md-4 control-label">Principal Location Address</label>
                      <div class="col-md-6">
                        <textarea name="principal_name_address" id="principal_name_address" class="form-control addressfield" rows="6" onContextMenu="return false" style="resize:vertical;"><?=$row_locdet['principal_place_address']?></textarea>
                      </div>
                    </div>
                  </div>
                  <div class="form-group">
