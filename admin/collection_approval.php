    ...
    <div class="col-sm-9 tab-pane fade in active table-responsive" id="home">
      ...
      <form class="form-horizontal" role="form" name="form1" id="form1" action="" method="post">
        <div class="form-group">
          <div class="col-sm-6 col-md-6 col-lg-6"><label class="col-sm-5 col-md-5 col-lg-5 control-label">From Date</label>
       ...
       <table  width="98%" id="myTable" class="table-striped table-bordered table-hover" align="center">
          <thead>
            <tr class="<?=$tableheadcolor?>" >
              ...
              <th data-hide="phone">Collection Date</th>
          ...
          <tbody>
			...
			if($_SESSION["userid"]=="admin"){
				$sql=mysqli_query($link1,"Select * from party_collection where ".$filter_str." order by id desc");
			}else{
				$sql=mysqli_query($link1,"Select * from party_collection where ".$filter_str." AND user_id IN (SELECT username FROM admin_users WHERE reporting_manager='".$_SESSION["userid"]."') order by id desc");
			}
            ...
            <tr class="even pointer">
              <td><?php echo $sno;?></td>
              <td><?php echo $userdet[0]." (".$row['user_id'].")";?></td>
              <td><?php echo $userdet[1];?></td>
              <td><?php echo str_replace("~",",",getLocationDetails($row['party_code'],"name,city,state,asc_code",$link1));?></td>
              <td><?php echo $row['transaction_date'];?></td>
              <td><?php echo $row['entry_date'];?></td>
              <td><?php echo $row['amount'];?></td>
              <td <?php if($row['status']=="Pending"){ echo "class='red_small'";}?>><?php echo $row['status'];?></td>
              <td align="center"><a href='collectionAppPage.php?id=<?php echo base64_encode($row['id']);?><?=$pagenav?>'  title='view'><i class="fa fa-eye fa-lg" title="view details"></i></a></td>
            </tr>
            <?php }?>
          </tbody>
          </table>
